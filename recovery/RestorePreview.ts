...
/** Minimal snapshot shape used for diff/risk calculations */
...
export class RestorePreview {
	 ...
	 */
	generateDiff(snapshot: SnapshotData | null | undefined): FileDiff {
		if (!snapshot) {
	 ...
	 */
	computeSnapshotRisk(config: SnapshotData | null | undefined): number {
		let risk = 0;
