...
const STORAGE_KEYS: Record<SecureConfigKey, string> = {
	"api.key": "vreko.secure.apiKey",
	"mcp.authToken": "vreko.secure.mcpAuthToken",
	"mcp.apiKey": "vreko.secure.mcpApiKey",
};
			...
			if (legacyValue && !deprecationWarningsShown.has(key)) {
				...
				logger.warn(
					`[DEPRECATED] Reading "${key}" from VS Code settings is deprecated. ` +
						`Run "Vreko: Migrate API Keys to Secure Storage" to migrate.`,
				);

				// Show user notification (once per session)
				void vscode.window
					.showWarningMessage(
						"Vreko: API keys in VS Code settings are deprecated and may be synced or visible. " +
					...
					.then((selection) => {
						if (selection === "Migrate") {
							void vscode.commands.executeCommand("vreko.migrateSecureConfig");
						}
					});
			}
		...
		async migrateAll(): Promise<number> {
			...
			if (migrated > 0) {
				vscode.window.showInformationMessage(
					`Vreko: Successfully migrated ${migrated} API key(s) to secure storage.`,
				);
			} else {
				vscode.window.showInformationMessage("Vreko: No API keys to migrate.");
			}
