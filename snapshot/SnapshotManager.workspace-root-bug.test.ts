 ...
 *
 * Example:
 * - Extension activated in /Vreko-Site
 * - Later tested in /portfolio-site
 * - Snapshot operations still use /Vreko-Site paths → WRONG
 */
...
describe('SnapshotManager - Workspace Root Caching Bug', () => {
	it('should use current workspace root, not cached construction-time value', () => {
		 ...
		 *
		 * Scenario:
		 * 1. Extension loads in workspace A (/Vreko-Site)
		 * 2. SnapshotManager created with workspaceRoot = "/Vreko-Site"
		 * 3. Workspace changes to workspace B (/portfolio-site)
		 * 4. SnapshotManager still uses "/Vreko-Site" (WRONG!)
		 *
		...
		// const workspaceA = '/users/test/Vreko-Site';
