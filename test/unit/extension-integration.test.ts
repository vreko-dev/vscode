...
describe.skip("VS Code Extension (legacy integration) [GH-ext-legacy]", () => {
	...
	it("should create checkpoint when command is executed", async () => {
		const extension = await import("../../src/extension");
		const mockContext = {
			subscriptions: [],
			globalStorageUri: { fsPath: "/tmp" },
	...
	it("should not create checkpoint for low-risk documents", async () => {
		const extension = await import("../../src/extension");
		const mockContext: vscode.ExtensionContext = {
			subscriptions: [],
			globalStorageUri: { fsPath: "/tmp" },
	...
	it("should auto-checkpoint protected files on save", async () => {
		const extension = await import("../../src/extension");
		const mockContext = {
			subscriptions: [],
			globalStorageUri: { fsPath: "/tmp" },
