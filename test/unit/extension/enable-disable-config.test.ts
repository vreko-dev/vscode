 ...
 *
 * Verifies that the snapback.enabled configuration setting correctly
...
describe("StatusBarController - Enable/Disable Configuration", () => {
	describe("Configuration Change Handling", () => {
		it("should enable StatusBarController when snapback.enabled is set to true", () => {
			// Mock VS Code workspace configuration
			const mockConfig = {
				get: vi.fn((key: string, defaultValue?: boolean) => {
					if (key === "snapback.enabled") return true;
					return defaultValue;
				}),
			};
			...
			const handleConfigChange = (e: { affectsConfiguration: (key: string) => boolean }) => {
				if (e.affectsConfiguration("snapback.enabled")) {
					const enabled = mockWorkspace.getConfiguration().get("snapback.enabled", true);
					if (enabled) {
			...
			handleConfigChange({
				affectsConfiguration: (key: string) => key === "snapback.enabled",
			});
		...
		it("should disable StatusBarController when snapback.enabled is set to false", () => {
			// Mock VS Code workspace configuration
			const mockConfig = {
				get: vi.fn((key: string, defaultValue?: boolean) => {
					if (key === "snapback.enabled") return false;
					return defaultValue;
				}),
			};
		...
		it("should not call enable/disable for unrelated configuration changes", () => {
			...
			handleConfigChange({
				affectsConfiguration: (key: string) => key === "snapback.vitals.showInStatusBar",
			});
		...
		it("should default to true when snapback.enabled is not set", () => {
		...
		it("should handle rapid toggle between enabled/disabled states", () => {
			...
			const mockConfig = {
				get: vi.fn((key: string, defaultValue?: boolean) => {
					if (key === "snapback.enabled") return configValue;
					return defaultValue;
				}),
			};
			...
			configValue = true;
			handleConfigChange({ affectsConfiguration: (key: string) => key === "snapback.enabled" });
	...
	describe("Configuration Schema Validation", () => {
		it("should have snapback.enabled defined with correct properties", () => {
			// This is a conceptual test - in real implementation, package.json is validated
			const expectedConfig = {
				type: "boolean",
				default: true,
				markdownDescription: expect.stringContaining("Enable SnapBack Extension"),
				order: 0,
			};
		...
		it("should prioritize snapback.enabled at order 0 (first in settings)", () => {
			// snapback.enabled should be the first setting (order: 0)
			const snapbackEnabledOrder = 0;
			const aiDetectionOrder = 1;

			expect(snapbackEnabledOrder).toBeLessThan(aiDetectionOrder);
		});
	});
	...
	describe("Logging Behavior", () => {
		it("should log when extension is enabled via config", () => {
			...
			const mockConfig = {
				get: vi.fn((key: string) => key === "snapback.enabled" ? true : undefined),
			};
			...
			const handleConfigChange = (e: { affectsConfiguration: (key: string) => boolean }) => {
				if (e.affectsConfiguration("snapback.enabled")) {
					...
					if (enabled) {
						mockStatusBarController.enable();
						mockLogger.info("SnapBack extension enabled via config", { enabled: true });
					} else {
						mockStatusBarController.disable();
						mockLogger.info("SnapBack extension disabled via config", { enabled: false });
					}
				}
			};
			...
			expect(mockLogger.info).toHaveBeenCalledWith("SnapBack extension enabled via config", { enabled: true });
		});
		...
		it("should log when extension is disabled via config", () => {
			...
			const mockConfig = {
				get: vi.fn((key: string) => key === "snapback.enabled" ? false : undefined),
			};
			...
			expect(mockLogger.info).toHaveBeenCalledWith("SnapBack extension disabled via config", { enabled: false });
		});
	});
});
