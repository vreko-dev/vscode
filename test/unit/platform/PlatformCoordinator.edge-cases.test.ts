...
describe("PlatformCoordinator - Edge Cases", () => {
	...
	describe("Issue 3.1: Workspace ID Conflicts", () => {
		it("should detect mismatch between manifest and SecretStorage", async () => {
			...
			await mockSecrets.store("snapback.workspaceId", "ws_secrets_id_12345678901234567890");
			...
			await fs.writeFile(path.join(snapbackDir, "workspace.json"), JSON.stringify(manifestWithDifferentId), "utf-8");
		...
		it("should handle copied .snapback directory from another machine", async () => {
			// Simulate user copying .snapback from Machine A to Machine B
			...
			await mockSecrets.store("snapback.workspaceId", localWorkspaceId);
			...
			await fs.writeFile(path.join(snapbackDir, "workspace.json"), JSON.stringify(foreignManifest), "utf-8");
		...
		it("should preserve tier information during workspace ID reconciliation", async () => {
			...
			await mockSecrets.store("snapback.workspaceId", localId);
			...
			await fs.writeFile(path.join(snapbackDir, "workspace.json"), JSON.stringify(proManifest), "utf-8");
	...
	describe("Issue 3.2: Schema Validation", () => {
		it("should reject manifest with invalid version number", async () => {
			...
			await fs.writeFile(path.join(snapbackDir, "workspace.json"), JSON.stringify(futureManifest), "utf-8");
		...
		it("should validate tier enum values", async () => {
			...
			await fs.writeFile(path.join(snapbackDir, "workspace.json"), JSON.stringify(invalidTierManifest), "utf-8");
		...
		it("should validate health status enum values", async () => {
			...
			await fs.writeFile(path.join(snapbackDir, "workspace.json"), JSON.stringify(invalidHealthManifest), "utf-8");
		...
		it("should require all mandatory fields", async () => {
			...
			await fs.writeFile(path.join(snapbackDir, "workspace.json"), JSON.stringify(incompleteManifest), "utf-8");
	...
	describe("Issue 3.3: Permission Errors", () => {
		...
		it("should handle read permission denied on existing manifest", async () => {
			...
			// Should fall back to creating new manifest
	...
	describe("Workspace ID Format Validation", () => {
		it("should reject manifest with invalid workspace ID format", async () => {
			...
			const invalidIds = [
				"ws_tooshort",
				"ws_UPPERCASE_NOT_ALLOWED_1234567890",
				"invalid_prefix_abc123def456789012345",
				"ws_has-dashes-abc123def456789012345",
				"",
			];
			...
			for (const invalidId of invalidIds) {
				...
				// Should reject invalid ID and generate new one
				expect(result.workspaceId).toMatch(/^ws_[a-f0-9]{32}$/);
