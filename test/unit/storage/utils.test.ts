...
import { generateSnapshotId, generateSessionId, generateAuditId, parseTimestampFromId, randomId } from '../../../src/storage/utils/fileId.js';
import { hashContent, getBlobPath } from '../../../src/storage/utils/hash.js';
import { atomicWriteFile, readJsonFile, writeJsonFile, ensureDirectory, fileExists } from '../../../src/storage/utils/atomicWrite.js';
...
describe('Storage Utilities', () => {
  ...
  beforeEach(async () => {
    tempDir = path.join(os.tmpdir(), `snapback-utils-test-${Date.now()}-${Math.random()}`);
  ...
  describe('fileId utilities', () => {
    ...
    it('should be Windows-safe (no colons in IDs)', () => {
      const ids = [generateSnapshotId(), generateSessionId(), generateAuditId()];
      ids.forEach(id => {
  ...
  describe('hash utilities', () => {
    ...
    it('should create blob path with 2-level structure', () => {
      ...
      expect(blobPath).toBe('ab/cd/abcd1234567890abcd1234567890abcd1234567890abcd1234567890abcd1234');
    });
    ...
    it('should extract directory levels from hash', () => {
      const hash = 'xyzabc1234567890abcd1234567890abcd1234567890abcd1234567890abcd12';
      const blobPath = getBlobPath(hash);
  ...
  describe('atomicWrite utilities', () => {
    ...
    it('should create parent directories recursively', async () => {
      const fileUri = vscode.Uri.joinPath(storageUri, 'deep/nested/path/file.txt');
      await atomicWriteFile(fileUri, 'content');

      expect(await fileExists(vscode.Uri.joinPath(storageUri, 'deep/nested/path'))).toBe(true);
