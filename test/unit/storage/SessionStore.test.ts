...
describe("SessionStore", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(
			os.tmpdir(),
			`snapback-session-test-${Date.now()}-${Math.random()}`,
		);
	...
	describe("Session Lifecycle", () => {
			...
			const files: SessionFileEntry[] = [
				{
					filePath: "/file1.ts",
					snapshotId: "snap-1",
					changeStats: { added: 5, deleted: 0 },
				},
				{
					filePath: "/file2.ts",
					snapshotId: "snap-2",
					changeStats: { added: 0, deleted: 2 },
				},
			];
		...
		it("should track session timestamps", async () => {
			const _startId = sessionStore.startSession();
	...
	describe("Session Queries", () => {
		...
		it("should list all sessions", async () => {
			for (let i = 0; i < 3; i++) {
				const _sessionId = sessionStore.startSession();
				const files: SessionFileEntry[] = [];
				await sessionStore.finalizeSession("manual", files);
	...
	describe("Statistics", () => {
		...
		it("should get most recent session", async () => {
			...
			const _recent = await sessionStore.finalizeSession("manual", files);
	...
	describe("Edge Cases", () => {
		...
		it("should handle session with large file count", async () => {
			...
			const files: SessionFileEntry[] = Array.from({ length: 100 }, (_, i) => ({
				...
				snapshotId: `snap-${i}`,
				changeStats: {
					added: Math.random() * 100,
					deleted: Math.random() * 50,
				},
			}));
