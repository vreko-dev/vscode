...
describe("SnapshotStore", () => {
	...
	beforeEach(async () => {
		tempDir = path.join(
			os.tmpdir(),
			`snapback-snapshot-test-${Date.now()}-${Math.random()}`,
		);
	...
	describe("Snapshot Creation", () => {
		...
		it("should store files as blob references", async () => {
			const files = new Map([["test.ts", "const test = true;"]]);
	...
	describe("Snapshot Queries", () => {
		it("should get snapshots for specific file", async () => {
			...
			expect(forFile1).toContainEqual(
				expect.objectContaining({ id: snap1.id }),
			);
			expect(forFile1).toContainEqual(
				expect.objectContaining({ id: snap2.id }),
			);

			const forFile2 = await snapshotStore.getForFile("file2.ts");
			expect(forFile2).toContainEqual(
				expect.objectContaining({ id: snap2.id }),
			);
			expect(forFile2).not.toContainEqual(
				expect.objectContaining({ id: snap1.id }),
			);
		});
