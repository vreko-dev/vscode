...
import { AuditLog } from '../../../src/storage/AuditLog.js';
import type { AuditEntry } from '../../../src/storage/types.js';
...
describe('AuditLog', () => {
  ...
  beforeEach(async () => {
    tempDir = path.join(os.tmpdir(), `snapback-audit-test-${Date.now()}-${Math.random()}`);
  ...
  describe('JSONL Format', () => {
    it('should store entries in JSONL format', async () => {
      ...
      // Read raw file
      const content = await fs.readFile(path.join(tempDir, 'audit.jsonl'), 'utf-8');
  ...
  describe('Query Operations', () => {
    ...
    it('should get entries by action type', async () => {
      ...
      expect(snapshots.every(e => e.action === 'snapshot_created')).toBe(true);
    });
    ...
    it('should handle time range with no matches', async () => {
      const future = await auditLog.getInRange(Date.now() + 100000, Date.now() + 200000);
  ...
  describe('Maintenance', () => {
    ...
    it('should rotate log when exceeding size', async () => {
      ...
      const originalSize = await auditLog.getSize();
  ...
  describe('Error Handling', () => {
    ...
    it('should handle malformed JSON lines gracefully', async () => {
      ...
      await fs.writeFile(path.join(tempDir, 'audit.jsonl'), '{invalid json\nnext line\n', 'utf-8');
    ...
    it('should handle mixed valid and invalid JSON', async () => {
      ...
      const currentContent = await fs.readFile(path.join(tempDir, 'audit.jsonl'), 'utf-8');
      await fs.writeFile(
