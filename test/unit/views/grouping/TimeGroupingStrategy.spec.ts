...
import type { SnapshotDisplayItem, TimeGroup } from '../../../../src/views/types.js';
  ...
  describe('group() - Time-based grouping', () => {
    ...
    it('should correctly group mixed timestamps', () => {
      ...
      const snapshots: SnapshotDisplayItem[] = [
        ...
        createSnapshot('snap-yesterday', new Date(now.getTime() - 25 * 60 * 60 * 1000)),
        createSnapshot('snap-thisweek', new Date(now.getTime() - 3 * 24 * 60 * 60 * 1000)),
        createSnapshot('snap-older', new Date(now.getTime() - 10 * 24 * 60 * 60 * 1000)),
      ];
