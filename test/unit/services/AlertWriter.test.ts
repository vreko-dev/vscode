...
describe("AlertWriter", () => {
	...
	describe("File System Operations", () => {
		it("should create .snapback directory if not exists", async () => {
			const alert = createAlert();

			await writer.write(alert);

			// Verify .snapback directory was created
			const alerts = await readAlerts();
			expect(alerts).toHaveLength(1);
		});
		...
		it("should append to existing alerts.jsonl file", async () => {
			...
			await mkdir(join(tempDir, ".snapback"), { recursive: true });
		...
		it("should handle nested directory creation", async () => {
			...
			const nestedAlertPath = join(nestedPath, ".snapback", "alerts.jsonl");
	...
	describe("Error Handling", () => {
		...
		it("should reject with error if directory creation fails", async () => {
			...
			const blockedPath = join(tempDir, ".snapback");
			await mkdir(tempDir, { recursive: true });
			// Create .snapback as a file instead of directory
