...
describe('ProtectionNotifications', () => {
  ...
  beforeEach(() => {
    ...
    mockGlobalState = {
      get: vi.fn((key: string, defaultValue?: any) => storage.get(key) ?? defaultValue),
  ...
  describe('showProtectionLevelNotification', () => {
    ...
    it('should not show notification if already acknowledged', async () => {
      ...
      await notifications.showProtectionLevelNotification('/path/file.ts', 'Warning', false);
    ...
    it('should always show notification for new protection regardless of acknowledgment', async () => {
      ...
      await notifications.showProtectionLevelNotification('/path/file.ts', 'Warning', true);
    ...
    it('should handle different protection levels independently', async () => {
      ...
      await notifications.showProtectionLevelNotification('/path/file.ts', 'Protected', false);
      expect(vscode.window.showInformationMessage).toHaveBeenCalled();
    });
  });
  ...
  describe('showProtectionLevelChanged', () => {
    it('should show notification for level changes', async () => {
      vi.mocked(vscode.window.showInformationMessage).mockResolvedValue('Got it' as any);
  ...
  describe('resetAcknowledgment', () => {
    ...
    it('should reset all levels when no level specified', async () => {
      ...
      await notifications.showProtectionLevelNotification('/path/file.ts', 'Warning', false);
      await notifications.showProtectionLevelNotification('/path/file.ts', 'Protected', false);
      await notifications.showProtectionLevelNotification('/path/file.ts', 'Watched', false);
  ...
  describe('emoji rendering', () => {
    it('should use correct emojis for protection levels', async () => {
      const calls: any[] = [];
      vi.mocked(vscode.window.showInformationMessage).mockImplementation((message: string): Promise<any> => {
        calls.push(message);
        return Promise.resolve('Got it');
      });

      await notifications.showProtectionLevelNotification('/path/file.ts', 'Watched', true);
      await notifications.showProtectionLevelNotification('/path/file.ts', 'Warning', true);
      await notifications.showProtectionLevelNotification('/path/file.ts', 'Protected', true);
