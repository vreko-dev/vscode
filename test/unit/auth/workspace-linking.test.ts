...
describe("Workspace Linking Flow", () => {
	...
	describe("Workspace ID Generation", () => {
		it("should generate valid workspace ID format", () => {
			// Format: ws_ + 32 hex characters
			const workspaceIdRegex = /^ws_[a-f0-9]{32}$/;
			...
			expect(id.length).toBe(35); // "ws_" (3) + 32 hex chars
		});
	...
	describe("Workspace Link API", () => {
		it("should link workspace to user successfully", async () => {
			...
			const response = await fetch("https://snapback-mcp.fly.dev/auth/link-workspace", {
				method: "POST",
				headers: {
	...
	describe("Tier Resolution via MCP", () => {
		it("should resolve pro tier for linked workspace", async () => {
			...
			const response = await fetch(
				`https://snapback-mcp.fly.dev/internal/resolve-tier?workspace_id=${workspaceId}`,
			);
		...
		it("should fall back to free tier for unlinked workspace", async () => {
			...
			const response = await fetch(
				`https://snapback-mcp.fly.dev/internal/resolve-tier?workspace_id=ws_unknown`,
			);
	...
	describe("End-to-End Flow Simulation", () => {
		it("should complete full auth → link → resolve flow", async () => {
			...
			// Auth
			const authResponse = await fetch("https://snapback.dev/api/deviceAuth/pollToken", {
				method: "POST",
			...
			// Link
			const linkResponse = await fetch("https://snapback-mcp.fly.dev/auth/link-workspace", {
				method: "POST",
	...
	describe("Error Handling", () => {
		it("should handle network failures gracefully", async () => {
			...
			try {
				await fetch("https://snapback-mcp.fly.dev/auth/link-workspace");
			} catch (error) {
...
describe("MCP Config URL Construction", () => {
	it("should build correct URL with workspace path", () => {
		...
		expect(url.toString()).toBe(
			"https://snapback-mcp.fly.dev/mcp?workspace=%2FUsers%2Fdev%2Fmy-project",
		);
	});

	it("should handle paths with spaces", () => {
		const baseUrl = "https://snapback-mcp.fly.dev/mcp";
		const workspacePath = "/Users/dev/My Projects/snapback";
