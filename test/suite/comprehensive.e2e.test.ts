...
suite("Vreko Extension End-to-End Test Suite", function () {
	...
	test("Should register all core commands", async () => {
		...
		const coreCommands = [
			...
			"vreko.setWarnLevel",
			"vreko.setBlockLevel",
			"vreko.showAllProtectedFiles",
			"vreko.deleteSnapshot",
			"vreko.renameSnapshot",
			"vreko.protectSnapshot",
			"vreko.viewSnapshot",
			"vreko.showAllSnapshots",
			"vreko.deleteOlderSnapshots",
			"vreko.unprotectAndDeleteSnapshot",
			"vreko.openWalkthrough",
			"vreko.refreshViews",
			"vreko.compareWithSnapshot",
			"vreko.updateConfiguration",
			"vreko.createPolicyOverride",
			"vreko.toggleOfflineMode",
		];
	...
	test("Should initialize the extension properly", async () => {
		...
		await vscode.commands.executeCommand("vreko.initialize");
	...
	test("Should protect a file with Watch level and create snapshot", async () => {
		...
		await vscode.commands.executeCommand("vreko.protectFile", testFileUri);
	...
	test("Should show protection status", async () => {
		...
		await vscode.commands.executeCommand("vreko.showStatus");
	...
	test("Should change protection level from Watch to Warn", async () => {
		...
		await vscode.commands.executeCommand("vreko.setWarnLevel", testFileUri);
	...
	test("Should change protection level from Warn to Block", async () => {
		...
		await vscode.commands.executeCommand("vreko.setBlockLevel", testFileUri);
	...
	test("Should view snapshots", async () => {
		...
		await vscode.commands.executeCommand("vreko.showAllSnapshots");
	...
	test("Should show all protected files", async () => {
		...
		await vscode.commands.executeCommand("vreko.showAllProtectedFiles");
	...
	test("Should rename a snapshot", async () => {
		...
		await vscode.commands.executeCommand("vreko.renameSnapshot");
	...
	test("Should delete a snapshot", async () => {
		...
		await vscode.commands.executeCommand("vreko.deleteSnapshot");
	...
	test("Should unprotect a file", async () => {
		...
		await vscode.commands.executeCommand("vreko.unprotectFile", testFileUri);
	...
	test("Should open walkthrough", async () => {
		...
		await vscode.commands.executeCommand("vreko.openWalkthrough");
	...
	test("Should refresh views", async () => {
		...
		await vscode.commands.executeCommand("vreko.refreshViews");
	...
	test("Should toggle offline mode", async () => {
		...
		await vscode.commands.executeCommand("vreko.toggleOfflineMode");
