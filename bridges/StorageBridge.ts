 ...
 * - Feature flagged: `vreko.useV2Engine` controls routing
...
import { Storage as V2Storage } from "../types/engine";
 ...
 *   useV2Engine: vscode.workspace.getConfiguration("vreko").get("useV2Engine", false),
 * });
 *
...
export class StorageBridge implements IStorageManager {
	...
	constructor(config: StorageBridgeConfig) {
		...
		if (config.useV2Engine) {
			// CRITICAL: fsPath can be undefined for non-file URIs (remote extension hosts)
			// Must validate before passing to path operations
			const storageFsPath = config.context.globalStorageUri?.fsPath;
			if (!storageFsPath || typeof storageFsPath !== "string") {
				logger.warn(
					"StorageBridge: globalStorageUri.fsPath is undefined - V2 engine not available for this workspace scheme",
					{
						scheme: config.context.globalStorageUri?.scheme,
					},
				);
				// Fall back to V1 storage instead of crashing
				this.config = { ...config, useV2Engine: false };
			} else {
	...
	async getSnapshot(id: string): Promise<SnapshotWithContent | null> {
		if (this.config.useV2Engine && this.v2Storage) {
			...
			const v2Manifest = await this.v2Storage.getSnapshot(id);
			if (v2Manifest) {
	...
	async listSnapshots(filters?: SnapshotFilters): Promise<SnapshotManifest[]> {
		if (this.config.useV2Engine && this.v2Storage) {
			// Get V2 snapshots
			const v2Snapshots = await this.v2Storage.listSnapshots();
	...
	async persistSnapshot(
		...
		trigger: SnapshotManifest["trigger"],
		options?: { name?: string; metadata?: SnapshotManifest["metadata"] },
	...
	private convertV2ToV1(
		v2: import("../types/engine").SnapshotManifest,
		trigger: SnapshotManifest["trigger"],
	): SnapshotManifest {
