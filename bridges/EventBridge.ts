/**
 * EventBridge - Maps @vreko/engine events to PostHog telemetry
 *
...
import type { VrekoEvents } from "../types/engine";
 ...
 *
 * Design:
 * - Subscribes to @vreko/engine event bus
...
export class EventBridge {
	...
	private readonly eventListeners: Array<{ event: string; listener: (payload: unknown) => void }> = [];
	...
	constructor(options: EventBridgeOptions) {
		...
		this.useV2 = options.useV2Engine ?? vscode.workspace.getConfiguration("vreko").get("useV2Engine", false);
	...
	/**
	 * Subscribe to all @vreko/engine events and forward to PostHog
	 */
	 ...
	 */
	private on<K extends keyof VrekoEvents>(event: K, handler: (payload: VrekoEvents[K]) => void): void {
		const listener = (payload: VrekoEvents[K]) => {
			try {
				handler(payload);
			} catch (error) {
		...
		this.eventListeners.push({ event: event as string, listener });
	}
