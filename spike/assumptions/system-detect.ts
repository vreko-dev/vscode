...
import { SpikeResult, timer } from '../utils';
...
export async function runSystemDetection(workspace: string): Promise<SpikeResult> {
  ...
  const { elapsed, result: systems } = await timer(async () => {
    ...
    if (fs.existsSync(pnpmWorkspace)) {
      ...
      const patterns = content.match(/- ['"]?([^'"]+)['"]?/g)?.map(m => m.replace(/- ['"]?/, '').replace(/['"]$/, ''));
      if (patterns) {
        for (const pattern of patterns) {
          const matches = await glob(pattern.replace('**', '*'), { cwd: workspace });
      ...
      if (fs.existsSync(dirPath)) {
        ...
        for (const entry of entries) {
          if (entry.isDirectory()) {
            ...
            if (hasPkg && !detected.some(d => d.path === path.join(dirPath, entry.name))) {
              const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'));
              detected.push({
  ...
  const foundApps = systems.filter(s => expectedApps.includes(path.basename(s.path)));
  ...
  return {
    ...
    metrics: {
      ...
      systems: systems.map(s => ({ name: s.name, type: s.type, method: s.method })),
    },
  };
}
