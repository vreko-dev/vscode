...
import { SpikeResult, timer, withTimeout } from '../utils';
...
export async function runMadgeTimeout(workspace: string): Promise<SpikeResult> {
  ...
  try {
    ...
    const { elapsed: fallbackElapsed, result: fallbackResult } = await timer(async () => {
      try {
        await withTimeout(
          new Promise((_, reject) => setTimeout(() => reject(new Error('simulated hang')), 100)),
          50 // Force timeout
        );
        return null;
      } catch {
    ...
    const fallbackNodeCount = fallbackResult ? Object.keys(fallbackResult).length : 0;
