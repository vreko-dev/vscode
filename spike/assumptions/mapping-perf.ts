...
export async function runMappingPerf(workspace: string): Promise<SpikeResult> {
  ...
  const mapping: SystemMapping = {
    systems: new Map([
      ...
      ['@snapback/vscode', { name: 'vscode', basePath: `${workspace}/apps/vscode` }],
      ['@snapback/core', { name: 'core', basePath: `${workspace}/packages/core` }],
      ['@snapback/sdk', { name: 'sdk', basePath: `${workspace}/packages/sdk` }],
      ['@snapback/platform', { name: 'platform', basePath: `${workspace}/packages/platform` }],
    ]),
    fileCache: new Map(),
  };
...
function getSystemForFile(filePath: string, mapping: SystemMapping, useCache: boolean): string | null {
