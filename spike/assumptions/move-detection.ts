...
export async function runMoveDetection(): Promise<SpikeResult> {
  ...
  // Test scenarios
  const scenarios: { name: string; events: FileEvent[]; expected: DetectedOperation[] }[] = [
    {
      name: 'True move (fast)',
      events: [
    ...
    },
    {
      ...
      expected: [{ type: 'move', from: '/src/a.ts', to: '/src/b.ts' }, { type: 'create', path: '/src/c.ts' }],
    },
  ];

  const results: { scenario: string; passed: boolean; actual: DetectedOperation[] }[] = [];
...
function detectOperations(events: FileEvent[], windowMs: number): DetectedOperation[] {
