...
export class ActivationFunnelIntegration {
	...
	constructor(config: ActivationFunnelConfig) {
		...
		const hasInstalled = this.context.globalState.get<boolean>("vreko.installed", false);
		if (hasInstalled && !this.context.globalState.get<boolean>("vreko.funnelCompleted", false)) {
	...
	trackInstalled(): void {
		...
		this.funnel.startFunnel(FunnelType.ACTIVATION);
		this.funnel.trackStep(FunnelType.ACTIVATION, PIONEER_EVENTS.EXTENSION_INSTALLED, {
	...
	trackAuthCompleted(provider?: string): void {
		...
		void this.context.globalState.update("vreko.funnelAuthCompleted", true);
	...
	/**
	 * Track a generic activation funnel step with optional properties.
	 * Used by pioneer funnel wiring for steps that don't have dedicated methods.
	 */
	trackStep(funnel: FunnelType, step: string, properties?: Record<string, unknown>): void {
		...
		this.funnel.trackStep(funnel, step, properties);
		logger.debug(`Funnel step tracked: ${funnel}/${step}`);
	}
