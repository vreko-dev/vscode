 ...
 *
 * @see Design: .qoder/quests/vreko-explorer-tree.md
...
export function createAuthedApiClient(context: vscode.ExtensionContext): AuthedApiClient {
	...
	return {
		async fetch<T>(path: string, init?: RequestInit): Promise<T> {
			...
			const apiKey = await context.secrets.get("vreko.apiKey");
			...
			// Create request with x-api-key header (Better Auth API key validation)
			...
			try {
				...
				if (response.status === 401 || response.status === 403) {
					...
					await context.secrets.delete("vreko.apiKey");
					throw new Error("Session expired - please reconnect your account");
				}
