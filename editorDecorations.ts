...
import type { FileChange } from "./types/engine";
import { detectThreats } from "./types/engine";
...
export class EditorDecorations {
	...
	private async updateDecorations(editor: vscode.TextEditor): Promise<void> {
		try {
			...
			// V2 Engine: Risk score - extension delegates to daemon
			// Default to 0 (safe) - daemon provides actual risk scoring
			const riskScore = 0;
			const threatResult = detectThreats(content);
