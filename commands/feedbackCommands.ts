 ...
 *
 * Commands:
 * - vreko.feedback.reportFalsePositive: User reports AI detection as false positive
 */
export function registerFeedbackCommands(context: vscode.ExtensionContext): vscode.Disposable[] {
	...
	const reportFalsePositiveDisposable = registerCommandSafely("vreko.feedback.reportFalsePositive", async () => {
		try {
	...
	logger.debug("Feedback command 'vreko.feedback.reportFalsePositive' registered");
