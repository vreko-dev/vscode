 ...
 * all network operations in the Vreko extension. Useful for restricted environments,
 * testing, or privacy-focused workflows.
 *
 * Commands:
 * - vreko.toggleOfflineMode: Toggle offline mode on/off
 *
...
export function registerOfflineModeCommands(
	 ...
	 *
	 * @command vreko.toggleOfflineMode
	 *
	 ...
	 * // Shows: "Vreko offline mode enabled" or "Vreko offline mode disabled"
	 * // Updates configuration at vreko.offlineMode.enabled
	 * ```
	 *
	 ...
	 * @since 1.2.0
	 */
	const toggleOfflineModeCommand = vscode.commands.registerCommand("vreko.toggleOfflineMode", async () => {
		try {
			...
			// Show notification
			vscode.window.showInformationMessage(`🦎 Vreko: Offline mode ${newOfflineMode ? "enabled" : "disabled"}`);
