 ...
 *
 * Commands:
 * - vreko.reviewSecurityIssues: Display security issues found in analysis
 * - vreko.blockSave: Block file save when critical issues are detected
 * - vreko.removeSecret: Prompt user to remove detected secrets
 * - vreko.removeMock: Prompt user to remove test mocks from production code
 * - vreko.addDependency: Prompt user to add missing dependencies
 *
...
export function registerDetectionCommands(
	 ...
	 *
	 * @command vreko.reviewSecurityIssues
	 *
	 ...
	 */
	disposables.push(
		vscode.commands.registerCommand("vreko.blockSave", async (_uri: vscode.Uri, analysisResult: AnalysisResult) => {
			const message =
	 ...
	 * ```
	 *
	 * @see {@link SecretDetectionPlugin} in @vreko/core for detection logic
	 * @todo Implement automatic secret removal for supported patterns
	 */
	disposables.push(
		vscode.commands.registerCommand("vreko.removeSecret", async (_uri: vscode.Uri, factor: string) => {
	 ...
	 * ```
	 *
	 * @see {@link MockReplacementPlugin} in @vreko/core for detection logic
	 * @todo Implement automatic mock removal for common frameworks
	 */
	disposables.push(
		vscode.commands.registerCommand("vreko.removeMock", async (_uri: vscode.Uri, factor: string) => {
	 ...
	 * ```
	 *
	 * @see {@link PhantomDependencyPlugin} in @vreko/core for detection logic
	 * @todo Implement automatic dependency addition via npm/yarn/pnpm
	 */
	disposables.push(
		vscode.commands.registerCommand("vreko.addDependency", async (_uri: vscode.Uri, factor: string) => {
