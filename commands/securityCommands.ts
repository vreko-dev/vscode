...
export function registerSecurityCommands(_context: vscode.ExtensionContext): vscode.Disposable[] {
	return [
		...
		vscode.commands.registerCommand("vreko.migrateSecureConfig", async () => {
			try {
		...
		vscode.commands.registerCommand("vreko.setApiKey", async () => {
			try {
				const apiKey = await vscode.window.showInputBox({
					title: "🦎 Vreko API Key",
					prompt: "Enter your 🦎 Vreko API key",
					password: true,
					placeHolder: "sk-...",
					validateInput: (value) => {
				...
				if (apiKey === "") {
					...
					vscode.window.showInformationMessage("🦎 Vreko: API key cleared.");
					logger.info("API key cleared by user");
				} else {
					await getSecureConfig().set("api.key", apiKey);
					vscode.window.showInformationMessage("🦎 Vreko: API key saved securely.");
		...
		vscode.commands.registerCommand("vreko.setMcpAuthToken", async () => {
			try {
				const token = await vscode.window.showInputBox({
					title: "🦎 Vreko MCP Auth Token",
