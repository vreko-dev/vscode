...
export function registerDecorationCommands(
	...
	disposables.push(
		vscode.commands.registerCommand("vreko.clearFileHealthDecorations", () => {
			try {
	...
	disposables.push(
		vscode.commands.registerCommand("vreko.refreshFileHealthDecorations", async () => {
			try {
				// 🆕 Check if file health decorations are enabled
				const config = vscode.workspace.getConfiguration("vreko");
				const showFileHealthDecorations = config.get<boolean>("showFileHealthDecorations", true);
	...
	disposables.push(
		vscode.commands.registerCommand("vreko.showFileHealthStatus", (uri: vscode.Uri | undefined) => {
			try {
				// 🆕 Check if file health decorations are enabled
				const config = vscode.workspace.getConfiguration("vreko");
				const showFileHealthDecorations = config.get<boolean>("showFileHealthDecorations", true);
