...
/**
 * QuickPick command for Vreko protection status
 *
...
async function showProtectionStatus(protectedFileRegistry: ProtectedFileRegistry): Promise<void> {
	...
	if (files.length === 0) {
		...
		if (result === "Protect a File") {
			await vscode.commands.executeCommand("vreko.protectFile");
	...
	const items: EnhancedQuickPickItem[] = [
		...
		},
		{
			...
			detail: "All files under 🦎 Vreko protection",
		},
		...
		// View all files
		{
			...
			description: "Show protected files list",
			detail: "Browse all protected files",
			action: async () => {
		...
		// Quick restore
		{
			...
			description: "Recent snapshots and restore",
			detail: "Quick access to snapshot timeline",
			action: async () => {
				await vscode.commands.executeCommand("vreko.showQuickPicker");
			},
		},
		...
		// Settings
		{
			label: "⚙️ Configure 🦎 Vreko",
			description: "Extension settings",
			detail: "Customize protection behavior and preferences",
			action: async () => {
				await vscode.commands.executeCommand("workbench.action.openSettings", "@ext:MarcelleLabs.vreko-vscode");
			},
		},
		...
		{
			label: "📖 Documentation",
			description: "Open Vreko console",
			detail: "View guides, tips, and best practices",
			action: async () => {
				await vscode.env.openExternal(vscode.Uri.parse("https://console.vreko.dev"));
			},
		},
	];
...
async function showFilesByLevel(
	...
	const filteredFiles = allFiles.filter(
		(f) =>
			...
			((level === "watch" || level === (undefined as unknown as typeof level)) && !f.protectionLevel),
	);
