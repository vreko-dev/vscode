...
> = {
	auth_failed: {
		title: "🦎 Vreko Cloud Sync",
		message: "Enable cloud sync to keep your snapshots synchronized across devices and share with your team.",
		icon: "🔐",
		actions: [
			{ label: "Authenticate", response: "authenticate", command: "vreko.authenticate" },
			{ label: "Learn More", response: "learn_more", command: "vreko.openDocs" },
		...
		whyItMatters:
			"Cloud sync keeps your intelligence layer continuous across devices and prevents local data loss.",
	},
	feature_discovered: {
		title: "🦎 Vreko Tip",
		message: "Did you know? 🦎 Vreko can automatically detect AI-generated code and protect your work.",
		icon: "💡",
		actions: [
			{ label: "Learn More", response: "learn_more", command: "vreko.openDocs" },
	...
	milestone_reached: {
		title: "🦎 Vreko Milestone",
		message: "🎉 You've created 10 snapshots! Your code is well protected.",
		icon: "🏆",
		actions: [
			{ label: "View Stats", response: "learn_more", command: "vreko.showDashboard" },
			{ label: "Got it", response: "not_now" },
		],
		educational: "Regular snapshots build muscle memory for code protection.",
	...
	session_health_warning: {
		...
		icon: "⚠️",
		actions: [
			{ label: "Create Snapshot", response: "create_snapshot", command: "vreko.createSnapshot" },
	...
	commit_recommended: {
		...
		actions: [
			{ label: "Commit Now", response: "commit_now", command: "workbench.action.git.commit" },
			{ label: "Create Snapshot First", response: "create_snapshot", command: "vreko.createSnapshot" },
			{ label: "Later", response: "not_now" },
		],
		educational:
		...
		whyItMatters:
			"GitClear research shows that PRs over 400 lines have 3x the defect rate. Committing now creates a safe snapshot before your changes grow further.",
...
export class NudgeManager {
	...
	async maybeNudge(trigger: NudgeTrigger): Promise<void> {
		...
		try {
			...
			const lastNudgeTime = this.context.globalState.get<number>("vreko.lastAuthNudge");
			const now = Date.now();
			...
			await this.context.globalState.update("vreko.lastAuthNudge", now);
	...
	getLastNudgeTime(): number | null {
		return this.context.globalState.get<number>("vreko.lastAuthNudge") ?? null;
	}
	...
	async showNudge(trigger: NudgeTrigger): Promise<NudgeResponse> {
		...
		if (selectedAction.response === "never") {
			await this.context.globalState.update(`vreko.nudge.${trigger}.disabled`, true);
	...
	isNudgeDisabled(trigger: NudgeTrigger): boolean {
		return this.context.globalState.get<boolean>(`vreko.nudge.${trigger}.disabled`) ?? false;
	}
	...
	async enableNudge(trigger: NudgeTrigger): Promise<void> {
		await this.context.globalState.update(`vreko.nudge.${trigger}.disabled`, false);
