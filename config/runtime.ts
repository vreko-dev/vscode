...
// read workspace.getConfiguration('vreko.guardian'); expose onDidChange + getters
...
export function initializeConfig() {
	...
	configChangeListener = vscode.workspace.onDidChangeConfiguration((e) => {
		if (e.affectsConfiguration("vreko.guardian")) {
			updateConfig();
		}
	});
}
...
function updateConfig() {
	const workspaceConfig = vscode.workspace.getConfiguration("vreko.guardian");
	config = {
