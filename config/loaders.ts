 ...
 */
export async function loadJsonConfig(filePath: string): Promise<VrekoRC> {
 ...
 */
export async function loadJson5Config(filePath: string): Promise<VrekoRC> {
	const content = await fs.readFile(filePath, "utf8");
 ...
 */
export async function loadYamlConfig(filePath: string): Promise<VrekoRC> {
	if (!yaml) {
		throw new Error("YAML support not available. Please install the yaml package.");
	}

	const content = await fs.readFile(filePath, "utf8");
	return yaml.parse(content) as VrekoRC;
}

/**
 * Load configuration from package.json
 */
export async function loadPackageJsonConfig(filePath: string): Promise<VrekoRC> {
	const content = await fs.readFile(filePath, "utf8");
	const packageJson = JSON.parse(content);
	return packageJson.vreko || {};
}
 ...
 */
export async function loadCjsConfig(filePath: string): Promise<VrekoRC> {
	// Check if executable configs are enabled
	if (!isExecutableConfigsEnabled()) {
		throw new Error(
			"CJS configuration loading is disabled by default. Enable with vreko.config.enableExecutableConfigs setting.",
		);
	}

	try {
		const result = await executeSandboxedScriptWrapper(filePath);
		return result.result as VrekoRC;
	} catch (error) {
 ...
 * OFF by default - must be explicitly enabled
 */
export async function loadMjsConfig(filePath: string): Promise<VrekoRC> {
	// Check if executable configs are enabled
	if (!isExecutableConfigsEnabled()) {
		throw new Error(
			"MJS configuration loading is disabled by default. Enable with vreko.config.enableExecutableConfigs setting.",
		);
	}
