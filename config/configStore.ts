/**
 * Vreko ConfigStore Wrapper
 *
 * Unified configuration management using @vreko/config
 * Replaces fragmented VSCode settings, VrekoRCLoader, and SettingsLoader
 *
 * This module provides the single point of config access for the VS Code Extension.
 */

import type { ConfigPath, ConfigStoreV2, PathValue } from "../types/config";
import { ConfigStore, getConfigStore } from "../types/config";
...
export async function initializeConfigStore(workspaceRoot: string): Promise<InstanceType<typeof ConfigStore>> {
	...
	try {
		...
		// Watch for .vrekorc changes in the background
