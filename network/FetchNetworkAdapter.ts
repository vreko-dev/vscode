 ...
 * const response = await adapter.get('https://api.vreko.dev/health');
 * if (response.ok) {
...
export class FetchNetworkAdapter implements NetworkAdapter {
	...
	async isOnline(): Promise<boolean> {
		...
		const healthEndpoints = [
			"https://api.vreko.dev/api/health", // API server (primary)
			"https://mcp.vreko.dev/health", // MCP server (fallback)
		];
		...
		for (const endpoint of healthEndpoints) {
			try {
				...
				// Log connectivity issues at debug level for troubleshooting
				logger.debug("Health check failed for endpoint", {
					endpoint,
