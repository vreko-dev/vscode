...
export class RulesManager {
	...
	private constructor(context: vscode.ExtensionContext) {
		...
		const publicKeyEnv = process.env.VREKO_RULES_PUBLIC_KEY;
		if (publicKeyEnv) {
	...
	private async fetchRules(): Promise<void> {
		...
		try {
			const api = VrekoAPI.getInstance();
	...
	public async validateRulesBundle(bundle: string): Promise<PolicyBundle> {
		try {
			...
			try {
				...
				if (!isValid) {
					const error = new Error("Invalid signature: bundle may be tampered");
					logger.error("Rule bundle signature verification failed");
					// Track signature verification failure with Error object
				...
				if (verifyError instanceof Error && verifyError.message?.includes("bad signature size")) {
					...
					// Track signature verification failure with Error object
					this.telemetry?.trackError(verifyError, {
						operation: "signature.verification",
