...
export class SnapshotQuickPicker implements vscode.Disposable {
	...
	constructor(
		...
		// Create MCP reader for .vreko/ directory
	...
	private async buildQuickPickItems(): Promise<SnapshotQuickPickItem[]> {
		...
		items.push({
			label: `${VREKO_ICONS.OVERVIEW} Open Dashboard`,
			description: "Full snapshot management & workspace vitals",
			action: "dashboard",
		});

		items.push({
			label: `${VREKO_ICONS.SESSION} Browse full history...`,
			description: "View all snapshots grouped by session",
			action: "browse",
		});

		items.push({
			label: `${VREKO_ICONS.SHIELD} Protection Status`,
		...
		if (snapshots.length > 0) {
			...
			items.push({
				label: `${VREKO_ICONS.IN_PROGRESS} No snapshots yet`,
	...
	private async showRestoreOptions(snapshot: SnapshotQuickPickItem): Promise<void> {
		...
		quickPick.onDidAccept(async () => {
			...
			if (selected?.snapshotId) {
				...
				await vscode.commands.executeCommand("vreko.restoreSnapshot", selected.snapshotId);
			}
		});
...
export function registerSnapshotQuickPickerCommands(
	...
	disposables.push(
		vscode.commands.registerCommand("vreko.showQuickPicker", async () => {
			await picker.show();
		}),
	);
	...
	disposables.push(
		vscode.commands.registerCommand("vreko.quickRestore", async () => {
			await picker.show();
		}),
	);
