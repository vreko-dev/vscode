...
export class ProtectionDecorationProvider implements vscode.FileDecorationProvider {
	...
	constructor(
		...
		this.disposables.push(
			registry.onProtectionChanged((uris) => {
				logger.info("[Vreko] Decoration provider received onProtectionChanged event for URIs:", uris);
	...
	private debounceDecorationUpdate(uris: vscode.Uri[], immediate = false): void {
		...
		if (immediate) {
			logger.info(`[Vreko] Firing immediate decoration update for ${uris.length} URIs`);
		...
		this.debounceTimer = setTimeout(() => {
			if (this.pendingUris.length > 0) {
				logger.info(`[Vreko] Firing debounced decoration update for ${this.pendingUris.length} URIs`);
	...
	provideFileDecoration(uri: vscode.Uri): vscode.FileDecoration | undefined {
		...
		if (isProtected) {
			...
			let tooltip = `Protected by Vreko (${levelMetadata.label})`;

			// If file has an active override, modify the decoration
			if (activeOverride) {
				badge = "⚡"; // Use lightning bolt to indicate override
				tooltip = `Overridden by Vreko (${levelMetadata.label}) - ${activeOverride.rationale}`;
			}

			// Only log protected files at debug level
			logger.debug(`[Vreko] Decorated protected file: ${uri.fsPath} (${levelMetadata.label})`);
