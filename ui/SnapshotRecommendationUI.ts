...
// REMOVED: TRAJECTORY_SIGNAGE - no longer needed (tooltip built by StatusFlagManager)
...
const EDUCATIONAL_MESSAGES: Record<string, { tip: string; whyItMatters: string }> = {
	...
	medium: {
		tip: "Good time to take a snapshot. Consistent snapshots give you reliable protection.",
...
export class SnapshotRecommendationUI implements vscode.Disposable {
	...
	/**
	 * Reference to StatusFlagManager for consolidated status bar updates
	 *
	 * DESIGN: We no longer create our own status bar item.
	 * Instead, we delegate to StatusFlagManager.showRecommendation()
	 * This consolidates all status bar state into a single item.
	 */
	private statusBarManager: StatusFlagManager | undefined;

	constructor(statusBarManager?: StatusFlagManager) {
		this.statusBarManager = statusBarManager;
	}

	/**
	 * Set StatusFlagManager reference (for deferred initialization)
	 */
	setStatusBarManager(manager: StatusFlagManager): void {
	...
	private updateStatusBar(recommendation: SnapshotRecommendation): void {
		...
		// Delegate to StatusFlagManager
		this.statusBarManager?.showRecommendation(recommendation.urgency, recommendation.reason);
	}
				...
				vscode.window
					...
					.then((selection) => {
						if (selection === "Learn More") {
							vscode.commands.executeCommand("vreko.openDocs");
						}
					});
				break;
			}
