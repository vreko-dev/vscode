...
import type { DaemonHealthStateType } from "@vreko/contracts/local-service";
...
export class StatusBarController implements vscode.Disposable {
	...
	constructor(
		...
		private readonly statusFlagManager: StatusFlagManager,
	) {
	...
	private handleHealthChange(health: SessionHealth): void {
		...
		// Always update StatusFlagManager with health/trajectory for tooltip
		const healthLevel = mapHealthLevel(healthScore);
		const trajectoryCanonical = mapTrajectory(trajectory);
		this.statusFlagManager.updateSessionHealth(healthLevel, trajectoryCanonical);
	...
	private updateStatusBarForState(state: StatusBarState): void {
		switch (state) {
			case "idle":
				this.statusFlagManager.showIdle();
				break;
			case "ambient-risk":
				// Subtle visual - handled via updateSessionHealth colors
				break;
			case "recommend":
				this.statusFlagManager.showRecommendation("medium", "Session pressure building - consider a snapshot");
				break;
			case "critical":
				this.statusFlagManager.showRecommendation("critical", "High risk - snapshot strongly recommended");
				break;
			default:
				break;
		}
	...
	public onSnapshotCreated(): void {
		...
		if (!["recovering", "error", "disabled"].includes(currentState)) {
			...
			this.statusFlagManager.showCheckpointCreated();
		}
	...
	/**
	 * Handle MCP health status change (from C1 MCPHealthMonitor via IPC).
	 *
	 * State mapping:
	 * - healthy → defer to health-score-based FSM (no override)
	 * - degraded → ambient-risk with tooltip override
	 * - recovering → ambient-risk with "recovering" message
	 * - failed → critical with doctor suggestion
	 */
	public onDaemonHealthChanged(state: DaemonHealthStateType): void {
		switch (state) {
			case "healthy":
				this.initializeFromCurrentHealth();
				break;
			case "degraded":
				this.stateMachine.transition("ambient-risk", "daemon_degraded");
				this.statusFlagManager.showRecommendation("medium", "Daemon health degraded");
				break;
			case "unhealthy":
				this.stateMachine.transition("critical", "daemon_unhealthy");
				this.statusFlagManager.showRecommendation("critical", "Daemon unhealthy  -  run 'vreko doctor'");
				break;
			case "unknown":
				this.stateMachine.transition("ambient-risk", "daemon_unknown");
				this.statusFlagManager.showRecommendation("medium", "Daemon connecting...");
				break;
		}
		logger.debug("[StatusBarController] Daemon health changed", { state });
	}

	/**
	 * Handle service becoming unresponsive.
	 */
	public onDaemonUnresponsive(): void {
		this.stateMachine.transition("critical", "daemon_unresponsive");
		this.statusFlagManager.showRecommendation("critical", "Daemon not responding");
		logger.warn("[StatusBarController] Daemon unresponsive");
	}

	/**
	 * Flash agent_active flag for 2s when an MCP tool call is detected.
	 * Called from extension.ts when daemonBridge.onMcpToolCalled fires.
	 */
	public handleMcpToolCalled(): void {
		if (vscode.workspace.getConfiguration("vreko.ui").get("statusBarEnabled", true)) {
			this.statusFlagManager.setFlag("agent_active");
		}
	}
...
export function createStatusBarController(
	...
	return new StatusBarController(dataService, statusFlagManager);
}
