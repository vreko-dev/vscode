...
/**
 * CodeLens provider for Vreko inline UI elements
 *
 ...
 */
export class VrekoCodeLensProvider implements vscode.CodeLensProvider {
	...
	provideCodeLenses(
		...
		try {
			...
			const protectionInfoLens = new vscode.CodeLens(new vscode.Range(0, 0, 0, 0), {
				...
				command: "vreko.showProtectionDetails",
				arguments: [filePath],
			});
	...
	register(context: vscode.ExtensionContext): void {
		...
		const allowOnceCommand = vscode.commands.registerCommand("vreko.allowOnce", (filePath: string) =>
			this.handleAllowOnce(filePath),
		);
		this.disposables.push(allowOnceCommand);

		const markWrongCommand = vscode.commands.registerCommand("vreko.markWrong", (filePath: string) =>
			this.handleMarkWrong(filePath),
		);
		this.disposables.push(markWrongCommand);

		const showDetailsCommand = vscode.commands.registerCommand("vreko.showProtectionDetails", (filePath: string) =>
			this.handleShowDetails(filePath),
		);
