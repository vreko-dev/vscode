 ...
 */
const TIER_STORAGE_KEY = "vreko.userTier";
const SNAPSHOT_COUNT_KEY = "vreko.lifetimeSnapshotCount";
...
export class UserTierService implements vscode.Disposable {
	...
	constructor(
		...
		this.disposables.push(
			vscode.workspace.onDidChangeConfiguration((e) => {
				if (e.affectsConfiguration("vreko.vitalsEnabled")) {
	...
	private showUpgradeNotification(tier: UserTier): void {
		const messages: Record<UserTier, string> = {
			...
			intermediate:
				"AI detection and session stats are now available. Continue using Vreko to access more features.",
			power: "All Vreko features are now available, including vitals and heat decorations.",
		};
		...
		if (message) {
			void vscode.window.showInformationMessage(message, "View Dashboard").then((selection) => {
				if (selection === "View Dashboard") {
					void vscode.commands.executeCommand("vreko.openDashboard");
				}
			});
		}
	}
