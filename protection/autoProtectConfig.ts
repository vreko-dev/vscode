...
/**
 * AutoProtectConfig - Automatic protection configuration
 *
 * Manages automatic protection rules from .vrekorc files.
 * Delegates to the CLI daemon via DaemonBridge for protection operations.
 *
...
export class AutoProtectConfig implements vscode.Disposable {
	...
	constructor(
		...
		private readonly decorator: VrekoRCDecorator,
	) {
		/* intentionally empty */
	}
	...
	private watchForCreation(): void {
		...
		this.watcher.onDidCreate(async (uri) => {
			...
			const choice = await vscode.window.showInformationMessage(
				"\u{1f7e1} .vrekorc created and auto-protected at Warn level",
				"Edit Configuration",
				"Learn More",
			);
			...
			if (choice === "Edit Configuration") {
				...
				await vscode.env.openExternal(vscode.Uri.parse("https://docs.vreko.dev/configuration"));
			}
		});
	}
		...
		this.protectedFileRegistry.onProtectionChanged((uris) => {
			...
			for (const uri of uris) {
				...
				if (level) {
					...
					vscode.window
						.showWarningMessage(
							"\u{26a0} .vrekorc should remain protected to prevent accidental changes",
							"Re-protect",
						)
						.then(async (choice) => {
	...
	private async showProtectionNotification(): Promise<void> {
		const key = `vreko.configProtected:${this.workspaceRoot}`;
		if (this.context.workspaceState.get(key)) {
			return;
		}

		const choice = await vscode.window.showInformationMessage(
			"\u{1f7e1} .vrekorc is protected at Warn level to prevent accidental changes",
			"Got It",
			"Don't Show Again",
		);
	...
	private async insertConfigTemplate(document: vscode.TextDocument): Promise<void> {
		const template = `{
  // Vreko Configuration
  // Documentation: https://docs.vreko.dev/configuration
