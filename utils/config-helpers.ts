...
/**
 * Default values for all Vreko settings
 */
export const VREKO_CONFIG_DEFAULTS: VrekoConfigSchema = {
	testMode: false,
	apiBaseUrl: "https://api.vreko.dev",
	"api.baseUrl": "https://api.vreko.dev/api",
	"api.key": "",
	...
	"mcp.enabled": true,
	"mcp.serverUrl": "https://mcp.vreko.dev",
	"mcp.authType": "bearer",
	"mcp.authToken": "",
	"mcp.apiKey": "",
	"mcp.timeout": 5000,
...
export class TypedVSCodeConfig {
	constructor(
		...
		private readonly defaults: VrekoConfigSchema,
	) {
		/* intentionally empty */
	}
	 ...
	 * ```
	 */
	get<K extends keyof VrekoConfigSchema>(key: K): VrekoConfigSchema[K] {
		const config = vscode.workspace.getConfiguration(this.section);
		return config.get<VrekoConfigSchema[K]>(key, this.defaults[key]);
	}
	 ...
	 */
	async update<K extends keyof VrekoConfigSchema>(
		key: K,
		value: VrekoConfigSchema[K],
	 ...
	 */
	affectsConfiguration<K extends keyof VrekoConfigSchema>(e: vscode.ConfigurationChangeEvent, key: K): boolean {
...
/**
 * Get type-safe Vreko configuration
 *
 * @example
 * ```ts
 * const config = getVrekoConfig();
 ...
 * ```
 */
export function getVrekoConfig(): TypedVSCodeConfig {
	return new TypedVSCodeConfig("vreko", VREKO_CONFIG_DEFAULTS);
}
