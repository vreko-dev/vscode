 ...
 * - Coordinate health status from DaemonHealthConsumer
 * - Trigger celebration events for success moments
 * - Maintain extension state at `.vreko/extension-state.json`
 *
...
/** Manifest file name  -  extension-owned state only; daemon writes .vreko/workspace.json separately */
const MANIFEST_FILENAME = "extension-state.json";
...
export class PlatformCoordinator implements vscode.Disposable {
	 ...
	 * 1. Try to create .vreko/extension-state.json with wx flag (exclusive)
	...
	async initialize(surface: Surface, version: string): Promise<PlatformInitResult> {
		...
		const vrekoDir = join(this.workspaceRoot, ".vreko");
		await mkdir(vrekoDir, { recursive: true });

		this.manifestPath = join(vrekoDir, MANIFEST_FILENAME);
		this.backupPath = join(vrekoDir, `${MANIFEST_FILENAME}.backup`);
	...
	wireDaemonBridge(bridge: DaemonBridge): void {
		...
		this.disposables.push(
			bridge.onStateChange(async (event) => {
				...
				// Celebrate recovery only when recovering from an actual failure (degraded),
				// not on normal reconnects (reconnecting/disconnected → connected).
				if (event.previousState === "degraded" && event.state === "connected") {
	...
	private createCelebration(type: CelebrationType, data?: Record<string, unknown>): CelebrationEvent {
		const messages: Record<CelebrationType, string> = {
			workspace_initialized: "🎉 Vreko workspace initialized! Ready to protect your code.",
	...
	private async loadManifest(): Promise<WorkspaceManifest | null> {
		...
		const secretWorkspaceId = await this.context.secrets.get("vreko.workspaceId");
			...
			try {
				...
				if (code === "EROFS" || attempt === MAX_RETRY_ATTEMPTS - 1) {
					...
					void vscode.window.showWarningMessage(
						"Vreko: Unable to save workspace manifest. Running in memory-only mode. Your workspace configuration will not persist.",
					);
					return;
				}
