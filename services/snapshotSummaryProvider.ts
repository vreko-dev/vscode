...
import type { RichSnapshot, Snapshot } from "../types/snapshot";
import { logger } from "../utils/logger";
import type { SnapshotSummary, SnapshotSummaryProvider } from "../views/types";
...
export class StorageSnapshotSummaryProvider implements SnapshotSummaryProvider {
	...
	private async readAll(): Promise<Snapshot[]> {
		try {
			...
			logger.warn("StorageSnapshotSummaryProvider:listSnapshots failed", { error: String(error) });
			return [];
		}
	}
	...
	private toSummary(snapshot: Snapshot): SnapshotSummary {
		...
		const name = this.asString(snapshot.meta?.name) || this.asString((snapshot as RichSnapshot).name); // Fallback for RichSnapshot type
		const files = this.extractFiles(snapshot);
		const description = this.asString(snapshot.meta?.description);

		return {
			id: snapshot.id,
			label: name || `Snapshot ${new Date(snapshot.timestamp ?? snapshot.createdAt).toLocaleString()}`,
			createdAt: snapshot.timestamp ?? snapshot.createdAt,
			description,
