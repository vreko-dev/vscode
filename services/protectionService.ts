...
export class ProtectionService {
	constructor(
		...
		private setContext: (key: string, value: unknown) => Promise<void> | void,
	) {}
	 ...
	 *
	 * Sets context keys:
	 * - vreko.protectionStatus: "unprotected" | "partial" | "complete" | "error"
	 * - vreko.attentionCount: number of items needing user attention
	 */
	async refreshContextKeys(): Promise<void> {
		try {
			...
			await this.setContext("vreko.protectionStatus", audit.status);

			// Set attention count context
			const attentionCount = audit.attentionItems?.length ?? 0;
			await this.setContext("vreko.attentionCount", attentionCount);
	 ...
	 * - When Vreko view becomes visible
	 *
