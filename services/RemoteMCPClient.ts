...
export class RemoteMCPClient implements vscode.Disposable {
	...
	async connect(): Promise<void> {
		...
		try {
			...
			throw new Error(
				`MCP connection failed after ${this.maxRetries} attempts. Background reconnection started. Try: 1) Check network connectivity, 2) Run "Vreko: Diagnose MCP" command, 3) Restart VS Code. Original error: ${(error as Error).message}`,
			);
		}
	}
