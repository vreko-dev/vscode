...
import type { ActivityPersistenceService } from "../ActivityPersistenceService";
...
export class WorkspaceDataService implements vscode.Disposable {
	...
	private persistenceService: ActivityPersistenceService | undefined;
	...
	constructor(workspaceId: string, workspacePath: string, coordinator: SnapshotCoordinator) {
		...
		this.vrekoDir = path.join(workspacePath, ".vreko");
		...
		this.learningsService = new LearningsService(workspacePath);
		this.settingsService = new SettingsService();

		// Wire DaemonBridge to LearningsService for daemon-first data
		const daemonBridge = getDaemonBridge(workspaceId);
		if (daemonBridge) {
			this.learningsService.setDaemonBridge(daemonBridge);
		}
	...
	/**
	 * Set the activity persistence service
	 * This connects persisted events to the activity timeline
	 */
	setActivityPersistenceService(service: ActivityPersistenceService): void {
		this.persistenceService = service;
		this.activityService.setPersistenceService(service);
		logger.debug("WorkspaceDataService wired to ActivityPersistenceService");
	}
	...
	private initialize(): void {
		...
		// Initial data load from daemon (async, fire-and-forget)
		void this.learningsService.loadAll();
	...
	/**
	 * Push a daemon activity event into the timeline.
	 * Called by ActivityFeedBridge when ProjectionStore emits changes.
	 * Privacy contract: events contain metadata only (no code content).
	 */
	pushDaemonEvent(event: ActivityEvent): void {
		this.activityService.addActivityEvent(event);
	}
