...
export class ActivityService {
	...
	private persistenceService?: ActivityPersistenceService;
	...
	/**
	 * Set the persistence service to sync events from
	 */
	setPersistenceService(service: ActivityPersistenceService): void {
		this.persistenceService = service;
		// Sync existing persisted events
		this.syncFromPersistence();
		// Listen for future changes
		service.onDidChange(() => this.syncFromPersistence());
	}

	/**
	 * Sync events from persistence service
	 */
	private syncFromPersistence(): void {
		...
		const persistedEvents = this.persistenceService.getEvents();
		for (const persisted of persistedEvents) {
			// Map PersistedActivityEvent type to ActivityEvent type
			const typeMap: Record<string, ActivityEvent["type"]> = {
				...
				"risk-detected": "service-risk",
				"ai-detected": "ai-edit",
			};

			// Convert PersistedActivityEvent to ActivityEvent
			const event: ActivityEvent = {
				id: persisted.id,
				type: typeMap[persisted.type] || "auto-snapshot",
				file: persisted.file,
				timestamp: persisted.timestamp,
				details: persisted.fileCount ? `${persisted.fileCount} files` : undefined,
				source: persisted.source,
				fileCount: persisted.fileCount,
			};

			// Avoid duplicates
			if (!this.activityEvents.some((e) => e.id === event.id)) {
				this.activityEvents.push(event);
			}
		}

		// Sort by timestamp (newest first)
		this.activityEvents.sort((a, b) => b.timestamp - a.timestamp);
