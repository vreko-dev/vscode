...
import type { ProtectionLevel as SDKProtectionLevel } from "../types/protection";
...
const STORAGE_KEY = "vreko:protected-files";
...
export class ProtectedFileRegistry implements ProtectedFileProvider, Disposable {
	...
	constructor(
		...
		logger.info("[Vreko] ProtectedFileRegistry constructed", {
	...
	initializeSDKProtectionManager(sdkManager: IProtectionManager): void {
		...
		logger.info("[Vreko] SDK ProtectionManager initialized with cached files", {
	...
	private loadFilesFromStorage(): ProtectedFileEntry[] {
		...
		logger.info("[Vreko] Protected files loaded from storage", {
			storedCount: stored.length,
	 ...
	 * Required by VrekoTreeProvider IConfigManager interface
	 */
	...
	async add(filePath: string, options?: { snapshotId?: string; protectionLevel?: ProtectionLevel }): Promise<void> {
		...
		if (this.sdkProtectionManager) {
			...
			logger.debug("[Vreko] Synced protection to SDK", { absolutePath, level });
		}
		...
		logger.info("[Vreko] Adding protected file:", filePath);

		const uri = vscode.Uri.file(filePath);
		logger.info("[Vreko] Firing onProtectionChanged for:", uri.fsPath);
		this._onProtectionChanged.fire([uri]);

		// REFACTOR: Use extracted publishEvent helper
		this.publishEvent(VrekoEvent.FILE_PROTECTED, {
			filePath: normalized,
	...
	async remove(filePath: string): Promise<void> {
		...
		if (next.length !== entries.length) {
			...
			if (this.sdkProtectionManager) {
				...
				logger.debug("[Vreko] Synced unprotection to SDK", { absolutePath });
			}
			...
			// REQUIRED: Fire decoration update event
			logger.info("[Vreko] Removing protected file:", filePath);
			const uri = vscode.Uri.file(filePath);
			logger.info("[Vreko] Firing onProtectionChanged for removal:", uri.fsPath);
			this._onProtectionChanged.fire([uri]);

			// REFACTOR: Use extracted publishEvent helper
			this.publishEvent(VrekoEvent.FILE_UNPROTECTED, {
				filePath: normalized,
				timestamp: Date.now(),
			});
		}
	...
	async updateProtectionLevel(filePath: string, level: ProtectionLevel): Promise<void> {
		logger.info(`[Vreko] updateProtectionLevel called - path: ${filePath}, level: ${level}`);
		const entries = await this.read();
		const normalized = this.normalize(filePath);
		logger.info(`[Vreko] Normalized path: ${normalized}`);
		const existingIndex = entries.findIndex((item) => item.path === normalized);

		if (existingIndex >= 0) {
			logger.info(
				`[Vreko] Found entry at index ${existingIndex}, current level: ${entries[existingIndex].protectionLevel}`,
			);
			entries[existingIndex].protectionLevel = level;
			entries[existingIndex].lastProtectedAt = Date.now();
			logger.info(`[Vreko] Updated entry protectionLevel to: ${entries[existingIndex].protectionLevel}`);
			...
			if (this.sdkProtectionManager) {
				...
				this.sdkProtectionManager.protect(absolutePath, this.mapToSDKLevel(level));
				logger.debug("[Vreko] Synced protection level update to SDK", { absolutePath, level });
			}

			await this.write(entries);
			logger.info("[Vreko] Written to storage");
			// Refresh cache immediately after update
			this.cachedFiles = this.loadFilesFromStorage();
			logger.info(
				`[Vreko] Reloaded cache, cached file protection level: ${
	...
	clearAll(): void {
		...
		if (this.sdkProtectionManager) {
			...
			logger.debug("[Vreko] Synced clearAll to SDK", { count: this.cachedFiles.length });
		}
		...
		// REQUIRED: Fire decoration update event
		if (allUris.length > 0) {
			logger.info("[Vreko] Clearing all protected files, firing onProtectionChanged for all URIs");
	...
	isProtected(filePath: string): boolean {
		...
		if (this.sdkProtectionManager) {
			...
			logger.debug("[Vreko] isProtected delegated to SDK", {
				filePath,
				absolutePath,
				result,
			});
		...
		// ✨ IMPROVED: Capture stack trace to identify early callers
		const stack = new Error().stack?.split("\n").slice(2, 5).join("\n") || "(no stack)";
		logger.warn("[Vreko] isProtected called before SDK initialization - returning false (safe default)", {
			filePath,
			callerStack: stack,
		});
		return false;
	...
	getProtectionLevel(filePath: string): ProtectionLevel | undefined {
		...
		if (this.sdkProtectionManager) {
			...
			logger.debug("[Vreko] getProtectionLevel delegated to SDK", {
				filePath,
				level,
			});
		...
		const stack = new Error().stack?.split("\n").slice(2, 5).join("\n") || "(no stack)";
		logger.warn(
			"[Vreko] getProtectionLevel called before SDK initialization - returning undefined (safe default)",
			{
				filePath,
				callerStack: stack,
			},
	...
	grantTemporaryAllowance(filePath: string, durationMs: number = THRESHOLDS.protection.otherCooldown): void {
		...
		logger.info(`[Vreko] Granted temporary allowance for ${normalized} (expires in ${durationMs}ms)`);
	}
	...
	hasTemporaryAllowance(filePath: string): boolean {
		...
		if (now > entry.expiresAt) {
			...
			logger.info(`[Vreko] Removed expired temporary allowance for ${normalized}`);
			return false;
		}
	...
	consumeTemporaryAllowance(filePath: string): boolean {
		...
		this.storageManager.removeCooldownByPath(normalized);
		logger.info(`[Vreko] Consumed temporary allowance for ${normalized}`);
		return true;
	}
}
