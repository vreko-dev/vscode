...
import { ExperienceClassifier } from "../utils/ExperienceClassifier";
...
class VSCodeStorageAdapter implements IKeyValueStorage {
	...
	async get<T>(key: string, defaultValue?: T): Promise<T | undefined> {
		if (defaultValue === undefined) {
			return this.globalState.get<T>(`vreko.experience.${key}`);
		}
		return this.globalState.get<T>(`vreko.experience.${key}`, defaultValue);
	}

	async set<T>(key: string, value: T): Promise<void> {
		await this.globalState.update(`vreko.experience.${key}`, value);
	}
}
...
export class UserExperienceService {
	...
	async getContextualHint(context: string): Promise<string | undefined> {
		...
		const hints: Record<string, string> = {
			firstSnapshot: "💡 Tip: Vreko automatically creates snapshots when you save protected files",
			firstProtection:
			...
			commandPalette: "💡 Tip: Press Ctrl+Shift+P (Cmd+Shift+P on Mac) to see all Vreko commands",
			treeView: "💡 Tip: Click the Vreko icon in the activity bar to see all your snapshots",
		};
	...
	async getRecommendedAction(): Promise<string | undefined> {
		...
		if (await this.isBeginner()) {
			if (metrics.snapshotsCreated === 0) {
				return "Try protecting your first file! Right-click any file and select 'Vreko: Protect File'";
			}
		...
		if (await this.isIntermediate()) {
			if (metrics.manualRestores === 0) {
				return "Learn how to restore snapshots! Open the Vreko view and try restoring a previous version";
			}
	...
	private async onLevelChanged(previousLevel: ExperienceLevel, newLevel: ExperienceLevel): Promise<void> {
		...
		await vscode.commands.executeCommand("setContext", "vreko.experienceLevel", newLevel);

		// Show congratulations message
		const messages: Record<ExperienceLevel, string> = {
			[ExperienceLevel.BEGINNER]: "", // No message for beginner
			[ExperienceLevel.INTERMEDIATE]:
				"🎉 You're now an intermediate Vreko user! More features are now available.",
			[ExperienceLevel.ADVANCED]: "All advanced features are now available.",
		};
	...
	async resetExperience(): Promise<void> {
		...
		const metrics: (keyof ExperienceMetrics)[] = [
			"snapshotsCreated",
			"sessionsRecorded",
			"protectedFiles",
			"manualRestores",
		...
		for (const metric of metrics) {
			await storage.set(metric as string, 0);
		}
		await storage.set("commandsUsed", {});
		await storage.set("firstUseTimestamp", Date.now());

		// Update VSCode context
		await vscode.commands.executeCommand("setContext", "vreko.experienceLevel", ExperienceLevel.BEGINNER);
