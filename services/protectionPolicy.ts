...
export interface ProtectionPolicy {
	...
	audit: {
		...
		loadedAt: number;
		/** Source of the policy ("defaults" | "vrekorc" | "merged") */
		source: "defaults" | "vrekorc" | "merged";
		/** Number of rules in the policy */
		rulesCount: number;
		/** Number of rules from defaults */
		defaultRulesCount: number;
		/** Number of rules from user .vrekorc */
...
export class ProtectionManager {
	...
	constructor(
		...
		private readonly getMergedConfig: () => VrekoRC | null,
	 ...
	 * Falls back to DEFAULT_VREKO_CONFIG when no user config is loaded
	 *
	...
	async getEffectivePolicy(): Promise<ProtectionPolicy | null> {
		...
		if (!mergedConfig) {
			...
			mergedConfig = DEFAULT_VREKO_CONFIG as VrekoRC;
		}
	 ...
	 */
	private async buildPolicy(mergedConfig: VrekoRC): Promise<ProtectionPolicy> {
		...
		return {
			...
			audit: {
				loadedAt: now,
				source: frameworks.length > 0 ? "merged" : "vrekorc",
					...
					for (const file of files) {
						...
						if (!currentLevel) {
							...
							items.push({
								...
								action: "vreko.setProtectionLevel",
							});
						}
					}
				} catch (error) {
