 ...
 *
 * NOTE: This class now uses the TreeItem factory pattern to ensure resourceUri
 * is always set when using ThemeIcon.File. This fixes the missing icon issue.
 *
...
export class SnapshotFileNode extends vscode.TreeItem {
	constructor(
		...
		this.command = {
			command: "vreko.openSnapshotFileDiff",
			title: "Open Diff",
			arguments: [this],
		};
	}
...
export class SnapshotNode extends vscode.TreeItem {
	constructor(
		...
		super(`Snapshot ${new Date(snapshot.timestamp ?? snapshot.createdAt).toLocaleString()}`, collapsibleState);
		this.contextValue = "snapshot";
		this.iconPath = new vscode.ThemeIcon("history");
		this.description = snapshot.id.substring(0, 8);
		this.tooltip = `ID: ${snapshot.id}\nCaptured: ${new Date(snapshot.timestamp ?? snapshot.createdAt).toLocaleString()}`;
	}
}
