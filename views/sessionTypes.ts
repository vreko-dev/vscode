...
import type { SessionFileEntry, SessionManifest } from "../snapshot/sessionTypes";
...
export class SessionFileTreeItem extends vscode.TreeItem {
	constructor(public readonly fileEntry: SessionFileEntry) {
		// Defensive: ensure fileEntry and uri exist
		if (!fileEntry || !fileEntry.uri) {
			super("unknown file", vscode.TreeItemCollapsibleState.None);
			this.tooltip = "Invalid file entry";
			this.contextValue = "sessionFileInvalid";
		...
		this.description = fileEntry.changeStats
			...
			: "";
		this.tooltip = `File: ${uri}\nSnapshot: ${fileEntry.snapshotId ?? "unknown"}`;
