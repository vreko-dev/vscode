...
/**
 * Mock tree item structure matching vrekoTreeProvider output
 */
...
describe("Snapshot Commands - Regression Tests", () => {
	...
	const DISPLAY_ID = `vreko:activity:snapshot:${ACTUAL_SNAPSHOT_ID}`;
	...
	describe("Tree Item ID Extraction", () => {
		it("should extract snapshot ID from item.data.id when called from tree view", () => {
			// Tree view item structure (from vrekoTreeProvider line 536)
	...
	describe("Regression: Tree Item ID Format", () => {
		 ...
		 * "Failed to delete snapshot: Snapshot not found: vreko:activity:snapshot:snap-1765962781479-bg6b2i"
		 *
		 * Root cause: Commands were using item.id directly, which is the display ID
		 * (set to `vreko:activity:snapshot:${snapshot.id}` for tree expansion persistence)
		 * instead of the actual snapshot ID stored in item.data.id
		 */
		it("should handle tree item ID format: vreko:activity:snapshot:*", () => {
			// This is the exact error case from the issue
			const displayId = "vreko:activity:snapshot:snap-1765962781479-bg6b2i";
			const actualId = "snap-1765962781479-bg6b2i";

			// Tree item from tree view (line 536 in vrekoTreeProvider.ts)
			const treeItem: MockTreeItem = {
				id: displayId,
				label: "📸 Activity Snapshot",
			...
			// Result: "Snapshot not found: vreko:activity:snapshot:snap-..."
			...
			expect(correctId).not.toContain("vreko:activity:snapshot:");
		});
