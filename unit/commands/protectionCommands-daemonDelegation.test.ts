...
import type { VrekoRCLoader } from "../../../src/protection/VrekoRCLoader";
...
describe("Protection Commands - Daemon Delegation", () => {
	...
	let mockVrekorcLoader: VrekoRCLoader;
	...
	beforeEach(() => {
		...
		commandContext = {
			...
			vrekorcLoader: mockVrekorcLoader,
	...
	describe("protectCurrentFile - Daemon Delegation", () => {
		it("should delegate to daemon when connected and workspace available", async () => {
			...
			const protectCommand = vi
				...
				.mock.calls.find((call) => call[0] === "vreko.protectCurrentFile")?.[1];
			...
			// Assert - VrekoRCLoader should NOT be called (daemon succeeded)
			expect(mockVrekorcLoader.addProtectionRule).not.toHaveBeenCalled();
		...
		it("should fall back to local when daemon fails", async () => {
			...
			// Assert - VrekoRCLoader should be called as fallback
			expect(mockVrekorcLoader.addProtectionRule).toHaveBeenCalledWith("/test/workspace/src/test.ts", "watch");
	...
	describe("unprotectFile - Daemon Delegation", () => {
		it("should NOT delegate to daemon in current implementation", async () => {
			...
			const unprotectCommand = vi
				.mocked(vscode.commands.registerCommand)
				.mock.calls.find((call) => call[0] === "vreko.unprotectFile")?.[1];

			// Act
			await unprotectCommand?.();

			// Assert - Documents current behavior (local only)
			expect(mockVrekorcLoader.removeProtectionRule).toHaveBeenCalledWith("/test/workspace/src/test.ts");
	...
	describe("setProtectionLevel - Daemon Delegation", () => {
		it("should delegate to daemon when connected and workspace available", async () => {
			...
			const setLevelCommand = vi
				.mocked(vscode.commands.registerCommand)
				.mock.calls.find((call) => call[0] === "vreko.setProtectionLevel")?.[1];
	...
	describe("changeProtectionLevel - Daemon Delegation", () => {
		...
		it("should delegate to daemon when connected and workspace available", async () => {
			...
			const changeLevelCommand = vi
				.mocked(vscode.commands.registerCommand)
				.mock.calls.find((call) => call[0] === "vreko.changeProtectionLevel")?.[1];
		...
		it("should fall back to local when daemon fails", async () => {
			...
			// Assert - VrekoRCLoader should be called as fallback
			expect(mockVrekorcLoader.addProtectionRule).toHaveBeenCalledWith("/test/workspace/src/test.ts", "block");
		});
	});
	...
	describe("Backward Compatibility", () => {
		it("should work when daemon is undefined", async () => {
			...
			expect(mockVrekorcLoader.addProtectionRule).toHaveBeenCalled();
			expect(vscode.window.showInformationMessage).toHaveBeenCalled();
		});
	});
		...
		describe("setWatchLevel", () => {
			it("should delegate to daemon when available", async () => {
				...
				const watchCommand = vi
					.mocked(vscode.commands.registerCommand)
					.mock.calls.find((call) => call[0] === "vreko.setWatchLevel")?.[1];
			...
			it("should fall back to local when daemon fails", async () => {
				...
				expect(mockVrekorcLoader.addProtectionRule).toHaveBeenCalledWith(
					"/test/workspace/src/test.ts",
					"watch",
				);
			});
		...
		describe("setWarnLevel", () => {
			it("should delegate to daemon when available", async () => {
				...
				const warnCommand = vi
					.mocked(vscode.commands.registerCommand)
					.mock.calls.find((call) => call[0] === "vreko.setWarnLevel")?.[1];
		...
		describe("setBlockLevel", () => {
			it("should delegate to daemon when available", async () => {
				...
				const blockCommand = vi
					.mocked(vscode.commands.registerCommand)
					.mock.calls.find((call) => call[0] === "vreko.setBlockLevel")?.[1];
