...
// Spy on all fs/promises exports while keeping original implementations
...
describe("PlatformCoordinator - Autonomous Recovery", () => {
	...
	describe("Issue 2.1: Silent Save Failures", () => {
		it("should retry manifest saves on transient failures", async () => {
			...
			vi.mocked(fs.writeFile).mockImplementation(async (...args: Parameters<typeof fs.writeFile>) => {
		...
		it("should notify user of persistent save failures", async () => {
			...
			vi.mocked(fs.writeFile).mockRejectedValue(Object.assign(new Error("EACCES: permission denied"), { code: "EACCES" }));
		...
		it("should attempt auto-recovery on next successful health check", async () => {
			...
			const manifestPath = path.join(testDir, ".vreko", "extension-state.json");
	...
	describe("Issue 2.2: Corrupted Manifest Recovery", () => {
		it("should recover from corrupted JSON using backup file", async () => {
			...
			// Create valid backup
			const backupPath = path.join(vrekoDir, "extension-state.json.backup");
			await fs.writeFile(
				backupPath,
				JSON.stringify({
		...
		it("should reconstruct manifest from SecretStorage if both files corrupted", async () => {
			...
			const vrekoDir = path.join(testDir, ".vreko");
			await fs.mkdir(vrekoDir, { recursive: true });
			await fs.writeFile(path.join(vrekoDir, "extension-state.json"), "corrupt", "utf-8");
			await fs.writeFile(path.join(vrekoDir, "extension-state.json.backup"), "also corrupt", "utf-8");

			// Store valid workspace ID in SecretStorage
			await mockSecrets.store("vreko.workspaceId", "ws_from_secrets_12345678901234567890");
		...
		it("should create fresh manifest as last resort", async () => {
			...
			const vrekoDir = path.join(testDir, ".vreko");
			await fs.mkdir(vrekoDir, { recursive: true });
			await fs.writeFile(path.join(vrekoDir, "extension-state.json"), "{corrupt json", "utf-8");

			const coordinator = new PlatformCoordinator(mockContext, testDir);
			const result = await coordinator.initialize("extension", "1.0.0");

			// Should create fresh manifest with unified 12-char format
	...
	describe("Disk Space and Permission Errors", () => {
		...
		it("should handle EACCES (permission denied) with user guidance", async () => {
			...
			vi.mocked(fs.mkdir).mockRejectedValue(Object.assign(new Error("EACCES: permission denied"), { code: "EACCES" }));
