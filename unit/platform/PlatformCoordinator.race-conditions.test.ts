...
describe("PlatformCoordinator - Race Conditions", () => {
	...
	describe("Issue 1.1: Concurrent Manifest Writes", () => {
		...
		it("should prevent manifest corruption from concurrent health updates", async () => {
			...
			// Verify structure integrity - uses unified 12-char format
			expect(manifest.workspaceId).toMatch(/^[a-f0-9]{12}$/);
		...
		it("should use atomic write operations to prevent partial writes", async () => {
			...
			const spyWriteFile = vi.mocked(fs.writeFile).mockImplementation(async (...args: Parameters<typeof fs.writeFile>) => {
				if (writeInProgress) {
					overlappingWrites++;
				}
	...
	describe("Double Wire Protection", () => {
		it.todo("should prevent duplicate health guardian wiring - wireHealthGuardian not yet implemented in PlatformCoordinator");
	});
});
