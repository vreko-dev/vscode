/**
 * @fileoverview VrekoRCLoader Error Handling Tests
 *
 * Tests for P1 UX improvement: User-friendly config error reporting
 * with "Open & Fix" action for .vrekorc JSON parse failures.
 *
...
describe("VrekoRC Loader Error Handling", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	});
	...
	describe("Notification Content", () => {
		it("should include file name and error location in message", () => {
			const rcPath = "/workspace/.vrekorc";
			const line = 5;
			const column = 12;

			const message = formatConfigErrorMessage(rcPath, line, column);

			expect(message).toContain(".vrekorc");
		...
		it("should mention default policy fallback", () => {
			const message = formatConfigErrorMessage("/workspace/.vrekorc", 3, 8);
	...
	describe("Logging Precedence Information", () => {
		it("should log config precedence when fallback is used", () => {
			...
			logConfigPrecedence(mockLogger, {
				...
				reason: ".vrekorc invalid",
				fallback: ".vreko/policy.json loaded",
			});

			expect(logEntries).toHaveLength(1);
			expect(logEntries[0].message).toContain("precedence");
			expect(logEntries[0].meta).toHaveProperty("activePolicy", "default");
			expect(logEntries[0].meta).toHaveProperty("reason", ".vrekorc invalid");
		});
	});
});

// Helper functions that mirror the implementation in VrekoRCLoader
...
function formatConfigErrorMessage(rcPath: string, line: number, column: number): string {
	const fileName = rcPath.split("/").pop() || ".vrekorc";
