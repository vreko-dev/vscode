...
// Mock fs module for .vreko directory reading
vi.mock("node:fs", () => ({
...
describe("WorkspaceDataService", () => {
	...
	describe("learnings and violations", () => {
		it("should return empty learnings when .vreko directory missing", async () => {
			const snapshot = await service.getSnapshot();

			expect(snapshot.learnings).toEqual([]);
		});

		it("should return empty violations when .vreko directory missing", async () => {
			const snapshot = await service.getSnapshot();

			expect(snapshot.violations).toEqual([]);
		});

		it("should return empty patterns when .vreko directory missing", async () => {
			const snapshot = await service.getSnapshot();
