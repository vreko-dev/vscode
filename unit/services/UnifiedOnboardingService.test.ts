	...
	describe("initialization", () => {
		...
		it("should migrate from legacy OnboardingProgression", async () => {
			...
			mockGlobalState.get.mockImplementation((key: string) => {
				if (key === "vreko.onboarding.unified") return undefined;
				if (key === "vreko:onboarding-state") {
					return {
						currentPhase: 3,
						snapshotsCreated: 5,
						hasRestored: false,
				...
				if (key === "vreko.milestones.snapshotCount") return 5;
				if (key === "vreko.events.first_snapshot_created") return true;
				return 0;
			});
		...
		it("should migrate to engaged state if 10+ snapshots exist", async () => {
			...
			mockGlobalState.get.mockImplementation((key: string) => {
				if (key === "vreko.onboarding.unified") return undefined;
				if (key === "vreko.milestones.snapshotCount") return 12;
				if (key === "vreko.events.first_snapshot_created") return true;
				return 0;
			});
	...
	describe("state transitions", () => {
		...
		it("should transition to value_demonstrated on first snapshot", async () => {
			...
			expect(mockNotificationManager.showNotification).toHaveBeenCalledWith(
				expect.objectContaining({
					icon: "🦎",
					message: "🦎 Vreko: Your first save is protected!",
				}),
			);
		...
		it("should transition to engaged at 10 snapshots", async () => {
			...
			expect(mockNotificationManager.showNotification).toHaveBeenCalledWith(
				expect.objectContaining({
					icon: "🦎",
					message: "🦎 Vreko: Protected 10 times!",
					actions: expect.arrayContaining([
						expect.objectContaining({
							title: "Join Pioneer Program",
							command: "vreko.pioneer.login",
						}),
					]),
				}),
			);
	...
	describe("celebration logic", () => {
		...
		it("should use 🦎 branding on all celebrations", async () => {
			...
			expect(mockNotificationManager.showNotification).toHaveBeenCalledWith(
				expect.objectContaining({
					icon: "🦎",
					message: expect.stringContaining("🦎 Vreko"),
				}),
			);
		});
