 ...
 *
 * TDD RED Phase: Tests for auto-injecting Vreko context into AI agent configs
 */

import { beforeEach, describe, expect, it, vi } from "vitest";
import {
	AgentConfigInjector,
	generateVrekoRulesContent,
	generateVrekoRulesWithFrontmatter,
	parseAgentConfig,
...
function createMockFileWriter(files: Record<string, string> = {}): IConfigFileWriter {
	...
	return {
		...
		ensureDir: vi.fn(async () => { /* intentionally empty */ }),
	};
}
...
describe("AgentConfigInjector", () => {
	...
	describe("generateVrekoRulesContent", () => {
		it("should generate content with injection marker", () => {
			const content = generateVrekoRulesContent();
			expect(content).toContain(VREKO_INJECTION_MARKER);
		});

		it("should include .vreko/ctx/context.json reference", () => {
			const content = generateVrekoRulesContent();
			expect(content).toContain(".vreko/ctx/context.json");
		});
	...
	describe("generateVrekoRulesWithFrontmatter", () => {
		it("should generate YAML frontmatter with version", () => {
			const content = generateVrekoRulesWithFrontmatter();
			expect(content).toContain("---");
			expect(content).toContain(`vrekoVersion: ${VREKO_RULES_VERSION}`);
		...
		it("should include markdown body after frontmatter", () => {
			...
			expect(parts[2]).toContain("Vreko Context Integration");
		});
	});
...
# Body`;
			...
			expect(parsed.frontmatter.vrekoVersion).toBe("1.0.0");
	...
	describe("injectForAgent", () => {
		...
		it("should merge with existing config file", async () => {
			...
			expect(writeCall[1]).toContain(VREKO_INJECTION_MARKER);
		});
...
# Already configured`;
			...
			expect(result.version).toBe(VREKO_RULES_VERSION);
...
# Old version`;
		...
		it("should re-inject if force=true", async () => {
			mockWriter = createMockFileWriter({
				"/test/workspace/.cursorrules": `${VREKO_INJECTION_MARKER}\n# Old version`,
			});
			injector = new AgentConfigInjector(mockWriter);
	...
	describe("hasVrekoInjection", () => {
		it("should detect existing injection", async () => {
			...
			const result = await injector.hasVrekoInjection(
