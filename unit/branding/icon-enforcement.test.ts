...
const ALLOWED_EMOJI_FILES = [
	...
	"constants/icons.ts", // VREKO_ICONS for MCP/dashboard specific icons
	"views/grouping/SystemGroupingStrategy.ts", // Tree view grouping uses codicons
	"views/grouping/FileGroupingStrategy.ts", // Tree view grouping uses codicons
	"ui/state-maps.ts", // State machine UI uses codicons
	"ui/ux-types.ts", // UX type definitions include codicon activity step templates
	"ui/StatusBarManager.ts", // Status bar uses codicons for animations
	"onboarding/stages/", // Onboarding stage steps use codicons for progress display
	"ui/DaemonStatusProvider.ts", // Daemon status uses codicons
	"signals/StatusFlagManager.ts", // Status flag manager uses codicons
	"signals/SignalCoordinator.ts", // Signal coordinator uses codicons for status display
	"mcp/auto-configure.ts", // MCP auto-configure uses codicons in quick pick
	"health/WorkspaceHealthMonitor.ts", // Health monitor uses codicons for status display
	"commands/recoveryCommands.ts", // Recovery commands use codicons in quick pick
];
