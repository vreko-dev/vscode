...
describe("J1 First-Time Activation Journey", () => {
	...
	describe("J1-E07: Corporate proxy blocks OAuth - Manual Token Entry", () => {
		...
		beforeEach(() => {
			manualTokenAuth = new ManualTokenAuthProvider("https://api.vreko.dev");
		});
		...
		it("should not report proxy when NO_PROXY includes vreko.dev", async () => {
			const originalEnv = process.env;
			process.env = {
				...originalEnv,
				HTTPS_PROXY: "http://proxy.corp.example.com:8080",
				NO_PROXY: "localhost,vreko.dev,.internal",
			};
		...
		it("should prompt for manual token with validation", async () => {
			...
			expect(vscode.window.showInputBox).toHaveBeenCalledWith(
				expect.objectContaining({
					title: "Vreko Manual Authentication",
					password: true,
