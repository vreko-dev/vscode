...
vi.mock("vscode", () => ({
	workspace: {
		...
		onDidSaveTextDocument: vi.fn(() => ({ dispose: () => { /* intentionally empty */ } })),
		onDidCreateFiles: vi.fn(() => ({ dispose: () => { /* intentionally empty */ } })),
		onDidDeleteFiles: vi.fn(() => ({ dispose: () => { /* intentionally empty */ } })),
		onDidChangeTextDocument: vi.fn(() => ({ dispose: () => { /* intentionally empty */ } })),
	},
	EventEmitter: class {
...
describe("MCPClient", () => {
	...
	describe("initialization", () => {
		...
		it("should generate formatted workspace ID", () => {
			...
			// Unified workspace ID format: 12-character hex (no ws_ prefix)
			expect(formatted).toMatch(/^[a-f0-9]{12}$/);
		});
...
describe("MCPClient Integration", () => {
	...
	describe("payload format", () => {
		it("should format payload correctly for remote server", async () => {
			...
			// Accepts unified 12-char format or legacy 32-char format
			expect(body.workspaceId).toMatch(/^[a-f0-9]{12}$|^ws_[a-f0-9]{32}$/);
