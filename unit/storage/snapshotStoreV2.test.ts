...
describe("SnapshotStore V2", () => {
	...
	beforeEach(async () => {
		vi.clearAllMocks();
		tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'vreko-test-'));
	...
	afterEach(async () => {
		vi.resetModules();
		if (tempDir) {
			await fs.rm(tempDir, { recursive: true, force: true }).catch(() => { /* intentionally empty */ });
		}
	});
	// ═══════════════════════════════════════════════════════════════════════════
	...
	describe("Edge Cases", () => {
		...
		it("should mark PRE as non-orphan when linked POST exists", async () => {
			const SnapshotStoreModule = await import("../../../src/storage/SnapshotStore");
