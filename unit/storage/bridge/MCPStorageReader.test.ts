 ...
 *
 * Tests for reading snapshots from MCP storage (.vreko/snapshots/)
 */
...
describe("MCPStorageReader", () => {
	...
	describe("list", () => {
		it("should return empty array when .vreko directory does not exist", async () => {
	...
	describe("getSnapshotDir", () => {
		it("should return correct path", () => {
			expect(reader.getSnapshotDir()).toBe(
				path.join(workspaceRoot, ".vreko", "snapshots"),
			);
		});
	});

	describe("exists", () => {
		it("should return true when .vreko directory exists", async () => {
			vi.mocked(fs.access).mockResolvedValue(undefined);
		...
		it("should return false when .vreko directory does not exist", async () => {
			vi.mocked(fs.access).mockRejectedValue(new Error("ENOENT"));
	...
	describe("getBlobContent", () => {
		it("should return blob content when file exists", async () => {
			...
			expect(fs.readFile).toHaveBeenCalledWith(
				path.join(workspaceRoot, ".vreko", "blobs", "ab", "abc123"),
			);
		});
