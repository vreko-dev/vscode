...
describe("ContextFileManager", () => {
	...
	describe("initialize", () => {
		it("should create context directory", async () => {
			...
			expect(fs.mkdir).toHaveBeenCalledWith(path.join(workspaceRoot, ".vreko", "ctx"), { recursive: true });
		});
		...
		it("should write schema file if missing", async () => {
			...
			expect(fs.writeFile).toHaveBeenCalledWith(
				path.join(workspaceRoot, ".vreko", "ctx", "context.schema.json"),
				expect.any(String),
				"utf-8",
			);
		});
		...
		it("should create initial context if file missing", async () => {
			...
			expect(fs.writeFile).toHaveBeenCalledWith(
				path.join(workspaceRoot, ".vreko", "ctx", "context.json"),
	...
	describe("live state updates", () => {
		it("should update on snapshot created", async () => {
			let snapshotHandler: () => void = () => { /* intentionally empty */ };
