 ...
 * from @vreko/sdk, enabling SessionCoordinator to work in VSCode.
 */

import * as vscode from "vscode";
import type { IDisposable, IEventEmitter } from "../types/sdk";
...
export class VscodeEventEmitterAdapter<T> implements IEventEmitter<T> {
	...
	/**
	 * Implement IEventEmitter.on - register event handler
	 * @param event - Event name (ignored for single-event emitter)
	 * @param handler - Function to call when event is fired
	 * @returns Disposable to unsubscribe
	 */
	on(_event: string, handler: (data: T) => void): IDisposable {
		return this.subscribe(handler);
	}

	/**
	 * Implement IEventEmitter.emit - fire event with data
	 * @param event - Event name (ignored for single-event emitter)
	 * @param data - Event data
	 */
	emit(_event: string, data: T): void {
		this.fire(data);
	}
