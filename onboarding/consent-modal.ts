...
/**
 * Consent modal for Vreko extension
...
export namespace ConsentModal {
	...
	export async function showConsentModal(): Promise<boolean> {
		const message = "🦎 Vreko: Welcome  -  Intelligent Code Protection";

		const detail = `🦎 Vreko helps protect your code by:
• Monitoring file changes for security risks
• Creating snapshots of your work
• Providing AI-powered code analysis

To function properly, 🦎 Vreko needs permission to:
...
You can change these settings anytime in VS Code Settings.`;
	...
	export async function hasGivenConsent(): Promise<boolean> {
		const config = vscode.workspace.getConfiguration("vreko.privacy");
	...
	export async function showClipboardConsent(): Promise<boolean> {
		const message = "🦎 Vreko: Clipboard Access";
		const detail =
			"🦎 Vreko: Needs clipboard access to provide context-aware AI assistance. This helps analyze code snippets and provide better protection recommendations.";
	...
	export async function showWatcherConsent(): Promise<boolean> {
		const message = "🦎 Vreko: File Watching";
		const detail =
			"🦎 Vreko: Needs to watch your file system to monitor changes and provide real-time protection. This helps detect potential security issues as you work.";
	...
	export async function showGitWrapperConsent(): Promise<boolean> {
		const message = "🦎 Vreko: Git Integration";
		const detail =
			"🦎 Vreko: Can integrate with Git to provide commit-time protection and workflow enhancements. This helps ensure your code is protected before it's committed.";
