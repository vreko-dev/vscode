			...
			if (analysisResult.factors?.length > 0) {
				...
				if (analysisResult.severity === "critical" || analysisResult.severity === "high") {
					const blockAction = new vscode.CodeAction(
						"_vreko: Block Save (Critical Security Issue)",
						vscode.CodeActionKind.QuickFix,
					);
					blockAction.command = {
				...
				for (const factor of analysisResult.factors) {
					if (factor.includes("secret") || factor.includes("password") || factor.includes("key")) {
						const action = new vscode.CodeAction(
							`_vreko: Remove Secret - ${factor.substring(0, 50)}${factor.length > 50 ? "..." : ""}`,
							vscode.CodeActionKind.QuickFix,
						);
						action.command = {
						...
						const action = new vscode.CodeAction(
							`_vreko: Remove Mock - ${factor.substring(0, 50)}${factor.length > 50 ? "..." : ""}`,
							vscode.CodeActionKind.QuickFix,
						);
						action.command = {
						...
						const action = new vscode.CodeAction(
							`_vreko: Add Dependency - ${factor.substring(0, 50)}${factor.length > 50 ? "..." : ""}`,
							vscode.CodeActionKind.QuickFix,
						);
						action.command = {
