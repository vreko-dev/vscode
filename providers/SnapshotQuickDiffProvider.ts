...
import { SNAPSHOT_SCHEME } from "../constants/uriSchemes";

/**
 * SnapshotQuickDiffProvider - QuickDiffProvider for "pre-AI" state tracking
 *
 * Provides original resource URIs (vreko-snapshot:) to VSCode's diff gutter
 ...
 * - Returns vreko-snapshot: URIs when tracked, null otherwise
 ...
 *   vscode.workspace.registerQuickDiffProvider('vreko-pre-ai', provider)
 * );
 *
 ...
 * // Returns: vreko-snapshot:snap-123/src%2Fauth.ts
 * ```
 *
...
export class SnapshotQuickDiffProvider implements vscode.QuickDiffProvider {
	...
	provideOriginalResource(uri: vscode.Uri, _token: vscode.CancellationToken): vscode.Uri | null {
		...
		// Construct vreko-snapshot: URI
		// Format: vreko-snapshot:<snapshotId>/<encodedFilePath>
