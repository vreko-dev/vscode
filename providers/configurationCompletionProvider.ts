...
export class ConfigurationCompletionProvider implements vscode.CompletionItemProvider {
	provideCompletionItems(
		...
		if (!document.fileName.endsWith(".vrekorc")) {
			return [];
		}
	...
	private getPatternCompletions(): vscode.CompletionItem[] {
		...
		return patterns.map((pattern) => {
			const item = new vscode.CompletionItem(pattern.label, vscode.CompletionItemKind.Value);
	...
	private getLevelCompletions(): vscode.CompletionItem[] {
		...
		return levels.map((level) => {
			const item = new vscode.CompletionItem(level.label, vscode.CompletionItemKind.EnumMember);
	...
	private getSettingsCompletions(): vscode.CompletionItem[] {
		const settings: Array<{
			...
			},
			{
				...
				detail: "number",
				defaultValue: "0",
				documentation: "Milliseconds between automatic snapshots (0 disables automatic snapshots).",
			},
			{
			...
			{
				label: "vreko.snapshot.maxSnapshots",
				detail: "number",
				defaultValue: "100",
				documentation: "Maximum number of snapshots to retain.",
			},
			{
				label: "vreko.snapshot.compressionEnabled",
				detail: "boolean",
				defaultValue: "true",
				documentation: "Enable compression for stored snapshots.",
			},
			{
				label: "vreko.snapshot.autoInterval",
				detail: "number",
				defaultValue: "0",
				documentation: "Milliseconds between automatic snapshots (0 disables automatic snapshots).",
			},
		...
		return settings.map((setting) => {
			const item = new vscode.CompletionItem(setting.label, vscode.CompletionItemKind.Property);
