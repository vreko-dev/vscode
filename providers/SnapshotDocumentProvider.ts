 ...
 * ┌─ URI Scheme: vreko-snapshot: ───────────────────────────────────────────┐
 * │ vreko-snapshot:src/components/auth.ts → snapshot content for auth.ts  │
 * │ vreko-snapshot:deep/nested/file.ts → snapshot content for file.ts     │
 * └────────────────────────────────────────────────────────────────────────────────┘
 *
 * ARCHITECTURE:
 ...
 *    vscode.workspace.registerTextDocumentContentProvider('vreko-snapshot', provider);
 *
 * 2. Set snapshot content:
 *    provider.setSnapshotContent('src/file.ts', 'snapshot content here');
 *
 * 3. Create virtual URI:
 *    const uri = vscode.Uri.parse('vreko-snapshot:src/file.ts');
 *
 ...
 *
 * @author Vreko Architecture Team
 * @version 1.0.0
 * @since 2024-01-09
 */
...
export class SnapshotDocumentProvider implements vscode.TextDocumentContentProvider {
	 ...
	 * a virtual document URI with the 'vreko-snapshot' scheme.
	 *
	 ...
	 *
	 * URI FORMATS:
	 * - Simple: vreko-snapshot:src/auth.ts
	 * - Composite: vreko-snapshot:snapshot-123/src/auth.ts
	 *
	 * ERROR HANDLING:
	 ...
	 *
	 * @param uri - Virtual document URI (scheme: vreko-snapshot)
	 ...
	 * const uri = vscode.Uri.parse('vreko-snapshot:src/auth.ts');
	...
	public provideTextDocumentContent(uri: vscode.Uri): string {
		...
		// For URI 'vreko-snapshot:src/auth.ts', path is 'src/auth.ts'
		// For URI 'vreko-snapshot:snapshot-123/src/auth.ts', path is 'snapshot-123/src/auth.ts'
	 ...
	 * const snapshotUri = vscode.Uri.parse('vreko-snapshot:src/auth.ts');
	...
	public setSnapshotContent(filePathOrSnapshotId: string, contentOrFilePath: string, optionalContent?: string): void {
		...
		const uri = vscode.Uri.parse(`vreko-snapshot:${uriPath}`);
