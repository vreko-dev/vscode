...
export class FeedbackManager {
	...
	/**
	 * StatusFlagManager for unified status bar (injected after initialization)
	 */
	private statusBarManager?: StatusFlagManager;
	...
	/**
	 * Inject StatusFlagManager for unified status bar
	 *
	 * Call this after StatusFlagManager is created in extension.ts
	 */
	public setStatusBarManager(manager: StatusFlagManager): void {
	...
	public handleDetection(detectionId: string, confidence: number) {
		...
		if (confidence > 0.8) {
			this.currentMessageId = this.statusBarManager.enqueueMessage({
				...
				text: "🤖 AI Detected",
				tooltip:
					"**AI Activity Detected**\nVreko is protecting this burst.\n\n*Incorrect? Click to report (+50 pts)*",
				duration: 0, // Persistent until dismissed
				command: "vreko.feedback.reportFalsePositive",
			});
		} else {
			this.currentMessageId = this.statusBarManager.enqueueMessage({
				id: `${FeedbackManager.MESSAGE_PREFIX}-${detectionId}`,
	...
	public dispose(): void {
		this.resetState();
		this.currentDetection = undefined;
		this.handledDetections.clear();
		this.statusBarManager = undefined;
		logger.debug("FeedbackManager disposed");
	}
}
