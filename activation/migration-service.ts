...
import { VREKO_ICONS } from "../constants/index";
...
export class MigrationService {
	private readonly MIGRATION_COMPLETED_KEY = "vreko.migration.completed";
	private readonly MIGRATION_ACTION_KEY = "vreko.migration.action";
	...
	private async showMigrationDialog(protectedCount: number): Promise<void> {
		const message = `Vreko has been updated! You have ${protectedCount} protected files from the previous version.

Previously, Vreko automatically protected files on startup. Now, protection requires your consent.
...
• Start Fresh: Clear all protections and protect only critical files`;
	...
	private async handleKeepAll(): Promise<void> {
		...
		vscode.window.showInformationMessage(
			`${VREKO_ICONS.SUCCESS} Migration complete! Your protected files remain unchanged.`,
		);
	}
	...
	private async handleStartFresh(): Promise<void> {
		...
		try {
			...
			vscode.window.showInformationMessage(
				`${VREKO_ICONS.SUCCESS} All ${protectedFiles.length} protected files cleared! You can now use "Protect This Repo" to protect only the files you need.`,
			);
		} catch (error) {
