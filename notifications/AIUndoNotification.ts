...
export class AIUndoNotification {
	...
	async show(event: AIUndoEvent, linesChanged = 0): Promise<boolean> {
		...
		const userPref = vscode.workspace.getConfiguration("vreko").get<boolean>("showAIChangeToasts", true);
		if (!userPref) {
		...
		const toolLabel = formatToolName(event.aiTool);
	...
	private async handleUndo(event: AIUndoEvent): Promise<void> {
		...
		try {
			...
			await vscode.commands.executeCommand("vreko.undoLastAIChange", event.snapshotId, event.filePath);
	...
	private async handleDisable(): Promise<void> {
		...
		await vscode.workspace.getConfiguration("vreko").update("showAIChangeToasts", false, true);

		vscode.window.showInformationMessage(
			"AI change notifications disabled. Re-enable in Settings > Vreko > Show AI Change Toasts.",
		);
	}
