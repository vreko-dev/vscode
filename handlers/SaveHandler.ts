...
import { type ClusterDetectionResult, RuntimeClusterService } from "../services/RuntimeClusterService";
...
import type { AIRiskService } from "../types/risk-types";
import type { ProtectionDecisionEngine } from "../types/sdk";
...
export class SaveHandler {
	...
	// Cluster detection via @vreko/core runtime
	private runtimeClusterService: RuntimeClusterService;

	// Legacy cluster cache (kept for backward compatibility)
	...
	constructor(
		...
		// Use provided AIRiskService or inline noop (daemon IPC handles real assessments via phase3)
		const riskService: AIRiskService = aiRiskService ?? {
			assessChange: async () => ({
				level: "low" as const,
				score: 0,
				confidence: 1,
		...
		// Initialize runtime cluster service (uses @vreko/core algorithms)
		this.runtimeClusterService = new RuntimeClusterService();
	...
	public initializeDecisionEngine(engine: ProtectionDecisionEngine): void {
		...
		logger.info("[Vreko] SDK ProtectionDecisionEngine initialized in SaveHandler");
	}
	...
	public setDaemonBridge(bridge: DaemonBridge): void {
		...
		logger.info("[Vreko] DaemonBridge wired into SaveHandler for cross-surface coordination");
	}
	...
	public initializeAlertSystem(
		...
		logger.info("[Vreko] Proactive Alert System initialized in SaveHandler");
	}
	...
	public async canSaveFile(
		...
		const clusterResult = await this.detectFileInCluster(filePath);
		const clusterAnchor = clusterResult.isInCluster ? clusterResult.anchorFile : undefined;

		// If file is in a cluster, check user tier
		if (clusterResult.isInCluster && clusterAnchor) {
		...
		return { allowed: true, clusterAnchor, requiresSnapshot: clusterResult.isInCluster };
	}
	 ...
	 *
	 * Uses @vreko/core runtime for session-level cluster detection.
	 * Groups snapshots into temporal sessions (30-min gap threshold).
	 *
	 * @param filePath - Absolute path to check
	 * @returns Cluster detection result with anchor file if in cluster
	 */
	public async detectFileInCluster(filePath: string): Promise<ClusterDetectionResult> {
		try {
			// Get recent snapshots from storage (last 24 hours)
			const recentSnapshots = await this.getRecentSnapshotsForClustering();

			// Use runtime cluster service for detection
			return await this.runtimeClusterService.detectFileInSessionCluster(filePath, recentSnapshots);
		} catch (error) {
			logger.error("Cluster detection failed", error as Error, { filePath });
			return { isInCluster: false };
		}
	}

	/**
	 * Get recent snapshots for clustering analysis
	 * Fetches snapshots from last 24 hours
	 */
	private async getRecentSnapshotsForClustering(): Promise<import("../storage/types").SnapshotManifest[]> {
		// This would typically come from storage manager
		// For now, return empty array - will be populated when storage integration is complete
		return [];
	}
	...
	register(context: vscode.ExtensionContext): void {
		const disposable = vscode.workspace.onWillSaveTextDocument((event) => {
			...
			const hasTrackedSave = context.globalState.get<boolean>("vreko.hasProtectedSave", false);
			...
			if (!hasTrackedSave && this.unifiedOnboarding) {
				...
				void (async () => {
					if (this.unifiedOnboarding) {
						...
						await context.globalState.update("vreko.hasProtectedSave", true);
		...
		const changeDisposable = vscode.workspace.onDidChangeTextDocument((event) => {
			...
			// This prevents recursive loops where Vreko's logging triggers AI detection
	...
	private async handleProtectedFileSave(
		...
		if (this.daemonBridge) {
			...
			const aiTool = aiDetectionResult.tool || undefined;

			// Fire-and-forget: daemon notification should not block save
			void this.daemonBridge
				.recordFileModification(workspacePath, filePath, linesChanged, isAiAttributed, aiTool)
				.then((success) => {
					if (success) {
