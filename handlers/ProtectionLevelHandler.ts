...
import { type SnapshotInfo, SnapshotNamingStrategy } from "../types/oss-sdk";
import type { EvaluationContext, ProtectionDecision, ProtectionDecisionEngine } from "../types/sdk";
...
export class ProtectionLevelHandler {
	...
	initializeDecisionEngine(engine: ProtectionDecisionEngine): void {
		...
		logger.info("[Vreko] ProtectionDecisionEngine initialized");
	}
	...
	async handleProtectionLevel(
		...
		const context: EvaluationContext = {
			filePath,
			protectionLevel,
			isAIDetected: !!aiDetection?.detected,
			riskScore: 0, // Will be calculated by decision engine
		};
		...
		if (decision.shouldSnapshot) {
			try {
				...
				if (snapshotId) {
					...
					// Derive shouldProceed from decision action
					const shouldProceed = decision.action === "allow" || decision.action === "warn";
				...
				vscode.window.showErrorMessage(`Vreko: Failed to snapshot ${filename}. Save will proceed.`);
				await this.auditLogger.recordAudit(filePath, protectionLevel, "save_allowed", {
...
A snapshot will be created before saving.`,
		...
		try {
			...
			vscode.window.showErrorMessage(`Vreko: Failed to create snapshot for ${filename}. Save will be blocked.`);
			await this.auditLogger.recordAudit(filePath, protectionLevel, "save_blocked", {
				reason: "snapshot_creation_failed",
				error: error instanceof Error ? error.message : String(error),
			});
	 ...
	 * This is the viral moment - when AI tries to delete/overwrite a file and Vreko catches it.
	 *
...
A snapshot will be created before saving.`,
	...
	private async createSnapshotForFile(
		...
		// Note: Line change computation was removed as it's not used by the simplified
		// SnapshotInfo structure. The naming strategy only needs path and content.
		...
		const snapshotInfo: SnapshotInfo = {
			id: `temp-${Date.now()}`, // Temporary ID for naming purposes
			origin: userContext || "manual",
			createdAt: Date.now(),
			files: [
				{
					path: relativePath,
		...
		if (snapshotId) {
			...
			if (coreTracker) {
				...
				// Note: storageManager is not in OperationCoordinator type but exists at runtime
				const coordinatorWithStorage = this.operationCoordinator as unknown as {
					storageManager?: { getActiveSessionId?: () => string };
				};
				const activeSessionId = coordinatorWithStorage.storageManager?.getActiveSessionId?.() || "unknown";
	...
	private showWarnNotification(filename: string, snapshotId: string, relativePath: string): void {
		...
		vscode.window.showInformationMessage(`Vreko captured a snapshot for "${filename}"`, "Restore Snapshot").then(
			async (selection) => {
				...
				try {
					...
					if (restored) {
						vscode.window.showInformationMessage(`Vreko restored "${filename}" from latest snapshot`);
					}
				} catch (error) {
					logger.error("Failed to restore warn-level snapshot", error as Error, { snapshotId, relativePath });
					vscode.window.showErrorMessage(`Vreko: Unable to restore ${filename} from snapshot`);
				}
			},
			(error: unknown) => {
