import { getGlobalProbeStore } from "@vreko/core/diagnostics/probe-store";
...
export class AnalysisCoordinator {
	...
	private probeStore: ReturnType<typeof getGlobalProbeStore> | null = null;
	...
	constructor(
		...
		this.diagnosticsEnabled = isDiagnosticsEnabled();
		if (this.diagnosticsEnabled) {
			this.probeStore = getGlobalProbeStore();
		}
	}
	...
	async analyzeAndPublish(
		...
		// Daemon reads file content locally  -  no content fields sent over IPC (ARCH-03)
		...
		// Record analysis probe for Path Attribution Gate
		if (this.diagnosticsEnabled && this.probeStore) {
			const impl = this.aiRiskService.constructor.name;
			const reason =
			...
			logger.debug("[Path Attribution] Analysis probe recorded", {
				capability: "analysis",
				impl,
				reason,
				file_path: filePath,
	...
	private async publishDiagnostics(
		...
		if (analysisResult.factors?.length > 0) {
			...
			analysisResult.factors.forEach((factor, index: number) => {
				...
				diagnostic.source = "Vreko";
				diagnostic.code = `vreko-${index}`;
...
class DiagnosticPublisher {
	private collection = vscode.languages.createDiagnosticCollection("vreko");
