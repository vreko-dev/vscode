...
export class Logger {
	...
	private constructor(outputChannel: vscode.LogOutputChannel) {
		...
		vscode.workspace.onDidChangeConfiguration((e) => {
			...
			if (e.affectsConfiguration("snapback.logNamespaces")) {
				this.config = vscode.workspace.getConfiguration("snapback");
				this.enabledNamespaces = new Set(this.config.get<string[]>("logNamespaces", []));
			...
			if (e.affectsConfiguration("snapback.logBatchDebug")) {
				this.config = vscode.workspace.getConfiguration("snapback");
				this.batchingEnabled = this.config.get<boolean>("logBatchDebug", false);
			}
		});
	 ...
	 *
	 * Child loggers allow filtering logs by subsystem. When `snapback.logNamespaces`
