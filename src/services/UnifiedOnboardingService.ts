...
import { CELEBRATIONS, formatCelebrationMessage, SNAPBACK_ICON } from "./onboarding/celebrations";
...
export class UnifiedOnboardingService {
	...
	private async showCelebration(
		...
		this.notificationManager.showNotification({
			id: `celebration-${type}-${Date.now()}`,
			type: "info",
			icon: SNAPBACK_ICON,
			message,
			detail: config.detail,
			timestamp: Date.now(),
			actions: config.action ? [config.action] : undefined,
		});
