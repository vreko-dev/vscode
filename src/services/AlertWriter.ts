 ...
 *
 * Writes alerts to `.snapback/alerts.jsonl` using append-only operations.
...
/**
 * ProactiveAlert type (until properly exported from @snapback/mcp)
 */
...
export class AlertWriter {
	...
	constructor(config: AlertWriterConfig) {
		...
		this.alertsPath = join(this.workspaceRoot, ".snapback", "alerts.jsonl");
	}
	 ...
	 *
	 * Ensures .snapback directory exists, then appends alert as JSONL line.
	 *
	 * @param alert - Alert to write
	...
	private async writeInternal(alert: ProactiveAlert, filePath: string): Promise<void> {
		try {
			...
			const dir = join(this.workspaceRoot, ".snapback");
