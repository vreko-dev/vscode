...
export function registerAuthCommands(
	...
	const connectCommand = vscode.commands.registerCommand("snapback.connect", async () => {
		try {
			...
			vscode.window.showErrorMessage(`SnapBack: Failed to open browser - ${(error as Error).message}`);
		}
	});

	// Register sign out command
	const signOutCommand = vscode.commands.registerCommand("snapback.signOut", async () => {
		...
		try {
			...
			vscode.window.showErrorMessage(`SnapBack: Failed to sign out - ${(error as Error).message}`);
		}
	});
