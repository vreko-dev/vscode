...
export function registerDecorationCommands(
	...
	disposables.push(
		vscode.commands.registerCommand("snapback.clearFileHealthDecorations", () => {
			try {
				...
				if (!showFileHealthDecorations) {
					vscode.window.showInformationMessage("File health decorations are disabled in settings");
					return;
				}

				commandContext.fileHealthDecorationProvider.clearAll();
				logger.info("File health decorations cleared");
				vscode.window.showInformationMessage("File health decorations cleared");
			} catch (error) {
				logger.error("Failed to clear file health decorations", error as Error);
				vscode.window.showErrorMessage("Failed to clear file health decorations");
			}
		}),
	);
	...
	disposables.push(
		vscode.commands.registerCommand("snapback.refreshFileHealthDecorations", async () => {
			try {
				// 🆕 Check if file health decorations are enabled
				const config = vscode.workspace.getConfiguration("snapback");
				const showFileHealthDecorations = config.get<boolean>("showFileHealthDecorations", true);
				...
				const protectedFiles = await commandContext.protectedFileRegistry.list();
				...
				for (const file of protectedFiles) {
					const protectionLevel = commandContext.protectedFileRegistry.getProtectionLevel(file.path);
					if (protectionLevel) {
						// Convert protection level to decoration format
						const decorationProtectionLevel =
							protectionLevel === "watch" ? "watch" : protectionLevel === "warn" ? "warn" : "block";
				...
				logger.error("Failed to refresh file health decorations", error as Error);
				vscode.window.showErrorMessage("Failed to refresh file health decorations");
			}
		}),
	);
	...
	disposables.push(
		vscode.commands.registerCommand("snapback.showFileHealthStatus", (uri: vscode.Uri | undefined) => {
			try {
				// 🆕 Check if file health decorations are enabled
				const config = vscode.workspace.getConfiguration("snapback");
				const showFileHealthDecorations = config.get<boolean>("showFileHealthDecorations", true);
				...
				const healthStatus = commandContext.fileHealthDecorationProvider.getFileHealth(targetUri);

				if (healthStatus) {
					const protectionLevel =
						commandContext.protectedFileRegistry.getProtectionLevel(targetUri.fsPath) || "Unknown";
