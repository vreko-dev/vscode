...
export function registerViewCommands(_context: vscode.ExtensionContext, ctx: CommandContext): vscode.Disposable[] {
	return [
		...
		vscode.commands.registerCommand("snapback.restoreSnapshot", async (snapshotId: string) => {
			if (!snapshotId) {
			...
			try {
				...
				const fileList =
					files.length <= 3 ? files.join(", ") : `${files.slice(0, 3).join(", ")} +${files.length - 3} more`;
...
This will overwrite current files.`,
				...
				if (result) {
					vscode.window.showInformationMessage(`✅ Restored ${files.length} file(s) from "${snapshotLabel}"`);
					ctx.refreshViews();
				} else {
