...
export function registerToggleGroupingModeCommand(
	...
	const command = vscode.commands.registerCommand("snapback.toggleGroupingMode", async () => {
		...
		if (selected?.enabled) {
			...
			vscode.window.showInformationMessage(`${selected.label} grouping is coming soon!`);
		}
	});
