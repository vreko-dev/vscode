 ...
 *
 * This is what makes SnapBack more than "fancy git stash."
 *
 * 🧢 SnapBack
 */
...
/**
 * Manages the .snapback/ctx/context.json file
 */
export class ContextFileManager implements Disposable {
	...
	constructor(
		...
		this.contextDir = path.join(workspaceRoot, ".snapback", "ctx");
	 ...
	 */
	async getContext(): Promise<SnapBackContext | null> {
		try {
	...
	private async writeContext(context: SnapBackContext): Promise<void> {
		const content = JSON.stringify(context, null, "\t");
		await fs.writeFile(this.contextPath, content, "utf-8");
	}

	private async readContext(): Promise<SnapBackContext | null> {
		try {
			const content = await fs.readFile(this.contextPath, "utf-8");
			return JSON.parse(content);
		} catch {
