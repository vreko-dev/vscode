...
export class ConfigMigration {
	 ...
	 */
	async migrate(targetVersion: string = CURRENT_CONFIG_VERSION): Promise<MigrationResult> {
			...
			for (const [version, migrateFn] of migrations) {
				try {
					...
					return {
						...
						toVersion: targetVersion,
						backupPath,
						error: migrationError instanceof Error ? migrationError : new Error(String(migrationError)),
					};
				}
			}
	 ...
	 */
	private getMigrationsToApply(fromVersion: string, toVersion: string): Array<[string, MigrationFn]> {
		const result: Array<[string, MigrationFn]> = [];

		for (const [version, migrateFn] of Object.entries(MIGRATIONS)) {
			if (this.compareVersions(version, fromVersion) > 0 && this.compareVersions(version, toVersion) <= 0) {
	...
	async loadConfigSafely(): Promise<unknown> {
		try {
			...
			if (error instanceof SyntaxError) {
				...
				throw new Error(`Config corrupted and recovery failed: ${recovery.error?.message}`);
			}
			throw error;
		}
	}
