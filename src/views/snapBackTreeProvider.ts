...
export class SnapBackTreeProvider implements vscode.TreeDataProvider<SnapBackTreeItem> {
  ...
  private getActionItems(): SnapBackTreeItem[] {
    const actions: QuickAction[] = [
      { id: 'create', label: 'Create Snapshot', icon: SNAPBACK_ICONS.CAMERA, command: COMMANDS.SNAPSHOT.CREATE },
      { id: 'restore', label: 'Restore Last', icon: SNAPBACK_ICONS.RESTORE, command: COMMANDS.SNAPSHOT.RESTORE_LEGACY },
      { id: 'search', label: 'Search Snapshots...', icon: SNAPBACK_ICONS.SEARCH, command: COMMANDS.SNAPSHOT.SHOW_ALL },
      { id: 'configure', label: 'Configure Protection', icon: SNAPBACK_ICONS.SETTINGS, command: COMMANDS.PROTECTION.PROTECT_WORKSPACE },
    ];
  ...
  ): { provider: SnapBackTreeProvider; view: vscode.TreeView<SnapBackTreeItem> } {
