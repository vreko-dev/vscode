...
// Use global mock from test/unit/setup.ts
// Do not define inline factory - it shadows global mock and causes inconsistencies
...
describe("SnapBackTreeProvider", () => {
	...
	beforeEach(() => {
		...
		mockStorageManager = {
			...
			getStorageUri: vi.fn().mockReturnValue({ fsPath: "/test/storage" }),
			setCooldown: vi.fn(),
			getCooldown: vi.fn(),
			isInCooldown: vi.fn(),
