...
export class BlockModalHandler {
  ...
  private pendingModals = new Map<string, { resolve: (action: BlockAction) => void; timer: ReturnType<typeof setTimeout> }>();
  ...
  private saveDontAskPreferences(): void {
    void this.context.globalState.update(
      this.config.dontAskKey,
      Array.from(this.dontAskFiles)
    );
  }
  ...
  async handleAutoSave(document: vscode.TextDocument): Promise<BlockAction> {
    ...
    if (this.config.showAutoSaveNotification) {
      ...
      vscode.window.setStatusBarMessage(
        `$(shield) SnapBack: Auto-snapshot for ${fileName}`,
        3000
      );
    }
