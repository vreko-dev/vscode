...
export class OfflinePointsQueue {
  ...
  private loadFromStorage(): void {
    try {
      ...
      const _pending = this.context.globalState.get<number>(PENDING_POINTS_KEY, 0);
      ...
      this.events = validEvents.length > this.config.maxSize
        ? validEvents.slice(-this.config.maxSize)
        : validEvents;
