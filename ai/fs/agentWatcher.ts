...
export class AgentWatcher {
			...
			if (result.score >= 8) {
				...
				if (!override) {
					...
					vscode.window.showErrorMessage(
						`Vreko blocked critical AI change in ${path.basename(filePath)} (Risk: ${result.score}/10)`,
					);
					return;
				}
				...
				vscode.window.showWarningMessage(
					`Vreko detected moderate risk (${result.score}/10) in AI change: ${path.basename(filePath)}`,
				);
			}
		} catch (error) {
	...
	private async requestOverride(score: number, factors: string[]): Promise<boolean> {
		...
		const result = await vscode.window.showWarningMessage(
			`Vreko detected a critical risk (${score}/10) in AI-generated change.${factorsText}`,
			{ modal: true },
			"Override",
			"Cancel",
		);
