 ...
 *
 * Automatically injects Vreko context rules into detected AI agent config files.
...
import { AgentConfigInjector } from "./AgentConfigInjector";
import { createNodeFileWriter } from "./NodeConfigFileWriter";
 ...
 * and offers to inject Vreko context rules into their configs.
 */
export async function autoConfigureAgentRules(context: vscode.ExtensionContext): Promise<void> {
	...
	try {
		...
		for (const agent of agents) {
			...
			if (configPath) {
				const result = await injector.hasVrekoInjection(configPath);
		...
		const response = await vscode.window.showInformationMessage(
			`Vreko detected ${agentNames}. Inject context rules for better AI assistance?`,
			"Inject Rules",
			"Not Now",
			"Never Ask",
		);
		...
		if (successful.length > 0) {
			...
			vscode.window.showInformationMessage(
				`✓ Vreko rules injected for ${names}. AI assistants will now read .vreko/ctx/context.json for context.`,
			);
		}
...
export function registerAgentRulesCommands(context: vscode.ExtensionContext): void {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("vreko.ai.injectRules", async () => {
			const workspaceFolders = vscode.workspace.workspaceFolders;
			if (!workspaceFolders) {
			...
			const items = await Promise.all(
				agents.map(async (agent) => {
					const configPath = injector.getConfigPathForAgent(agent, workspaceRoot);
					const result = configPath ? await injector.hasVrekoInjection(configPath) : { hasInjection: false };
					return {
						label: agent,
			...
			const selected = await vscode.window.showQuickPick(items, {
				canPickMany: true,
				placeHolder: "Select AI agents to configure with Vreko rules",
			});
	...
	// Reset command
	context.subscriptions.push(
		vscode.commands.registerCommand("vreko.ai.resetRulesConfig", async () => {
