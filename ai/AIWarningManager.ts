...
import { ok as Ok } from "@vreko/contracts";
import * as vscode from "vscode";
import { VREKO_ICONS } from "../constants/index";
import type { BurstDetectionResult } from "../types/sdk";
...
export class AIWarningManager {
	...
	private buildWarningMessage(detection: AIDetection, confidencePercent: number): string {
		const toolName = formatToolName(detection.tool);

		if (detection.burst) {
			// Include burst details if available
			const insertedChars = detection.burst.details?.totalInserted || 0;
			return (
				`${VREKO_ICONS.AI} AI-assisted edit detected (${toolName}, ${confidencePercent}% confidence)\n\n` +
