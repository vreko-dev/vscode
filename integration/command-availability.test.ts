 ...
 * See commit fe648f9: SnapshotQuickPicker called vreko.openVitalsDashboard but
...
describe("Command Availability", () => {
	...
	describe("Critical Commands", () => {
		it("should register vreko.openVitalsDashboard before QuickPicker references it", async () => {
			...
			// Command must exist
			expect(registeredCommands.has("vreko.openVitalsDashboard")).toBe(true);
		});

		it("should register vreko.showRecommendedAction for tips button", async () => {
			const { activate } = await import("../../src/extension");
			const mockContext = createMockExtensionContext();
			...
			expect(registeredCommands.has("vreko.showRecommendedAction")).toBe(true);
		});

		it("should register vreko.showWelcome for Intelligence empty state", async () => {
			// Regression test for SB-164: IntelligenceTreeProvider referenced showWelcome but it was never registered
			const { activate } = await import("../../src/extension");
			const mockContext = createMockExtensionContext();

			await activate(mockContext);

			// Command must exist for Intelligence view empty state
			expect(registeredCommands.has("vreko.showWelcome")).toBe(true);
		});
		...
		it("should register all QuickPicker action commands", async () => {
			...
			expect(registeredCommands.has("vreko.showQuickPicker")).toBe(true);
			expect(registeredCommands.has("vreko.quickRestore")).toBe(true);
		});
	});
	...
	describe("Command Execution Safety", () => {
		it("should throw user-visible error when command not found", async () => {
			...
			const executeCommand = vi.fn().mockRejectedValue(
				new Error("Command 'vreko.missingCommand' not found")
			);
			vscode.commands.executeCommand = executeCommand;

			try {
				await vscode.commands.executeCommand("vreko.missingCommand");
		...
		it("should validate all executeCommand calls have registered handlers", async () => {
			// Scan codebase for vscode.commands.executeCommand("vreko.*")
			...
			const unregisteredCommands = await findUnregisteredCommands(
				"/Users/user1/WebstormProjects/Vreko-Site/apps/vscode/src",
				registeredCommands
			);
	...
	describe("Command Registration Order", () => {
		it("should register vitals commands in Phase 4 before QuickPicker in Phase 5", async () => {
			...
			const vitalsIndex = registrationOrder.indexOf("vreko.openVitalsDashboard");
			const quickPickIndex = registrationOrder.indexOf("vreko.showQuickPicker");
	...
	describe("Progressive Disclosure Commands", () => {
		it("should register all progressive disclosure commands", async () => {
			...
			expect(registeredCommands.has("vreko.toggleAdvancedMode")).toBe(true);
			expect(registeredCommands.has("vreko.showAllFeatures")).toBe(true);
			expect(registeredCommands.has("vreko.resetExperienceLevel")).toBe(true);
			expect(registeredCommands.has("vreko.showRecommendedAction")).toBe(true);
		});
	});
});
	...
	return {
		...
		globalState: {
			get: vi.fn((key: string) => {
				...
				if (key === "vreko.experienceLevel") return "beginner";
				return undefined;
			}),
			update: vi.fn(),
			keys: vi.fn(() => []),
