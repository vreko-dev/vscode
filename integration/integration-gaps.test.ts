...
import { VrekoEventTypes } from "@vreko/contracts";
...
describe("Fix 3: EventBus SNAPSHOT_CREATED triggers TreeView refresh", () => {
	describe("Subscription is wired in extension.ts", () => {
		it("should subscribe to SNAPSHOT_CREATED event in extension.ts", async () => {
			...
			expect(content).toContain("VrekoEventTypes.SNAPSHOT_CREATED");
			expect(content).toContain("bus.on(VrekoEventTypes.SNAPSHOT_CREATED");
		});
	...
	describe("VrekoEventTypes enum", () => {
		it("should export SNAPSHOT_CREATED event type", () => {
			expect(VrekoEventTypes.SNAPSHOT_CREATED).toBe("snapshot:created");
		});
	});
});
