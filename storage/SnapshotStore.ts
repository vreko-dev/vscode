...
/**
 * SnapshotStore - Snapshot manifest storage
 *
 * Manages snapshot manifests with content-addressable blob storage.
...
import type { CreatePOSTOptions, CreatePREOptions } from "./snapshot-types";
import {
	addToIndex,
	allocateSeq,
	DEFAULT_INDEX,
...
// Re-export for any consumers that import these types from SnapshotStore
export type { CreatePOSTOptions, CreatePREOptions } from "./snapshot-types";
...
// ============================================
// Error Classes (re-exported from @vreko/contracts)
// ============================================

export { NoChangeError, SnapshotChainError } from "@vreko/contracts";
...
export class SnapshotStore {
	...
	async createPOST(options: CreatePOSTOptions): Promise<SnapshotManifestV2> {
		...
		// 🛡️ DEFENSIVE: Also check for paths that escape workspace
		// Normalize path to handle cases like "apps/vscode/../../packages/core/index.ts"
		const normalizedAnchor = path.normalize(options.anchorFile).replace(/\\/g, "/");

		// Check if normalized path escapes workspace (starts with '..' after normalization)
		if (normalizedAnchor.startsWith("..")) {
			logger.error("Anchor file path escapes workspace", {
				originalPath: options.anchorFile,
				normalizedPath: normalizedAnchor,
				storageRoot: this.storageUri.fsPath,
				filesInSnapshot: Array.from(options.files.keys()),
			});
			throw new Error(
